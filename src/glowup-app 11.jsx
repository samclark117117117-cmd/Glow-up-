...
const DAILY_RULES = [
  {id:'r01',emoji:'⏰',title:'No Snooze',description:'When your alarm goes off you get up immediately. Not in 5 minutes. Right now. The habit starts the second it goes off.'},
  {id:'r02',emoji:'💧',title:'Water Before Your Phone',description:'Before you look at your phone this morning drink a full glass of water. Your body needs it. Your phone can wait.'},
  {id:'r03',emoji:'📵',title:'No Phone for the First 30 Minutes',description:'Do not touch your phone for the first 30 minutes after waking up. Get up get moving be a person first.'},
  {id:'r04',emoji:'🛏',title:'Make Your Bed',description:'Make your bed the moment you get out of it. Before anything else. 2 minutes. Sets the tone for everything that follows.'},
  {id:'r05',emoji:'💄',title:'Full Appearance Before Leaving',description:'Hair makeup nails outfit perfume — all done before you leave the house. Every single time. No exceptions.'},
  {id:'r06',emoji:'💅',title:'Nails Always Done',description:'Your nails must always be painted and presentable. Bare chipped or undone nails are not an option. Keep on top of this.'},
  {id:'r07',emoji:'💧',title:'2.5 Litres of Water',description:'Hit your full 2.5 litre water target today. Not almost — the full amount. Track it if you need to.'},
  {id:'r08',emoji:'🥗',title:'Protein at Every Meal',description:'Every meal today has a proper protein source on the plate. No meal without it. This is non-negotiable for your progress.'},
  {id:'r09',emoji:'💪',title:'Show Up for Your Training Session',description:'Whatever your scheduled session is today you show up for it fully. No half effort no cutting it short. Start to finish.'},
  {id:'r10',emoji:'🛏',title:'In Bed by 10:30',description:'Lights out by 10:30. Sleep is when your body rebuilds and your mind resets. Everything else can wait. This cannot.'},
  {id:'r11',emoji:'📋',title:'Plan Tomorrow Tonight',description:'Before bed spend 5 minutes planning tomorrow. Wake time meals training. Know what the day looks like before it starts.'},
  {id:'r12',emoji:'🌸',title:'Skincare Morning and Night',description:'Full skincare routine every morning and every night. Cleanser moisturiser SPF in the morning. Full routine at night. No skipping.'},
];
...
function getWeekDates(weekStart) {
  var start = new Date(weekStart); var dates = [];
  for (var i=0;i<7;i++){var d=new Date(start);d.setDate(start.getDate()+i);dates.push(d.toDateString());}
  return dates;
}
function loadLocal() { try { return JSON.parse(localStorage.getItem(SK)) || {}; } catch(e) { return {}; } }
function saveLocal(d) { localStorage.setItem(SK, JSON.stringify(d)); }
...
function checkPerfectWeek(weekData, weekDatesArr) {
  ...
  for (var i=0;i<weekDatesArr.length;i++) {
    ...
    if (new Date(dateStr) > new Date(todayStr)) continue;
    var dayChallenges = weekData.challenges[dateStr] || [];
    if (dayChallenges.filter(function(c){return c.done;}).length < 3) return false;
  }
  for (var j=0;j<ACTIVE_TRAINING_DAYS.length;j++) {
    var dayId = ACTIVE_TRAINING_DAYS[j];
    var dayIndex = ['mon','tue','wed','thu','fri','sat','sun'].indexOf(dayId);
...
function pickChallenges(seen, seenDates, glowPoints, ovr, custom, naughtyActive, todayDayId) {
  var today = new Date().toDateString();
  var dow = new Date().getDay();
  var dayId = todayDayId || 'mon';
  var validCtx = DAY_CONTEXTS[dayId] || ['any'];
  var all = CHALLENGES.concat(custom || []);

  // Pinned challenges first
  var pinned = all.filter(function(c) {
    var o = ovr[c.id] || {};
    if (o.disabled) return false;
    if (c.category === 'Naughty' && !naughtyActive) return false;
    return o.rt === 'daily' || (o.rt === 'days' && (o.days||[]).indexOf(dow) >= 0) || (o.rt === 'date' && o.date === today);
  }).slice(0,3);
  if (pinned.length >= 3) return pinned.slice(0,3);
  ...
  // Filter by context, level, naughty flag, disabled
  var pool = all.filter(function(c) {
    var o = ovr[c.id] || {};
    if (o.disabled) return false;
    if (pids.indexOf(c.id) >= 0) return false;
  ...
  // Smart rotation: prefer not seen in last 7 days
  ...
  var stale = pool.filter(function(c){ return recentlySeen.indexOf(c.id) >= 0; });
  var src = fresh.length >= (3 - pinned.length) ? fresh : fresh.concat(stale);
