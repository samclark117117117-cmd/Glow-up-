    ...
    <div style={{minHeight:'100vh',background:T.bg,fontFamily:'Palatino Linotype,Palatino,Georgia,serif',position:'relative',overflow:'hidden'}}>
      ...
      <div style={{position:'relative',zIndex:1,maxWidth:520,margin:'0 auto',padding:'0 0 90px'}}>
        ...
        {tab==='rewards' && <div style={{padding:'0 15px',animation:'up .3s ease-out'}}>
          ...
          {st.rewards.length===0?<div style={{textAlign:'center',padding:'50px 20px',color:'rgba(255,179,217,.26)',fontStyle:'italic',fontFamily:'Crimson Text,serif',lineHeight:1.7}}>No rewards yet 🌸<br/>Sam will add some for you to unlock</div>:<div style={{display:'flex',flexDirection:'column',gap:12}}>{st.rewards.map(function(r){var isGlow=r.pointType==='glow';var balance=isGlow?st.glowPoints:st.grindPoints;var canAfford=balance>=r.cost;var redeemed=!!r.redeemed;var ptColor=isGlow?T.a1:'#ff3d9a';var ptLabel=isGlow?'✨ Glow Points':'💪 Grind Points';var minLvl=r.minLevel||0;var lvlReq=minLvl>0?GLOW_LEVELS.find(function(l){return l.level===minLvl;}):null;var locked=!!(lvlReq&&glowLvi.level<minLvl);return <div key={r.id} style={Object.assign({},cardS,{padding:'18px',border:redeemed?'1px solid rgba(255,120,190,.1)':locked?'1px solid rgba(255,120,190,.08)':canAfford?'1px solid '+T.a1+'44':'1px solid rgba(255,120,190,.13)',opacity:redeemed?0.45:locked?0.7:1})}><div style={{display:'flex',alignItems:'flex-start',gap:14}}><div style={{fontSize:'2rem',flexShrink:0}}>{r.emoji}</div><div style={{flex:1,minWidth:0}}><div style={{fontFamily:'Playfair Display,serif',fontWeight:'bold',fontSize:'1rem',color:redeemed?'rgba(255,179,217,.35)':'#ffe8f5',textDecoration:redeemed?'line-through':'none',marginBottom:4}}>{r.title}</div>{r.desc&&<p style={{margin:'0 0 8px',color:'rgba(255,179,217,.5)',fontSize:'0.84rem',fontFamily:'Crimson Text,serif',lineHeight:1.55}}>{r.desc}</p>}<div style={{display:'flex',alignItems:'center',gap:6,background:ptColor+'14',border:'1px solid '+ptColor+'30',borderRadius:20,padding:'4px 12px',marginBottom:8,width:'fit-content'}}><span style={{color:ptColor,fontWeight:'bold',fontFamily:'Playfair Display,serif',fontSize:'0.9rem'}}>{r.cost}</span><span style={{color:'rgba(255,179,217,.45)',fontSize:'0.65rem'}}>{ptLabel}</span></div>{locked&&lvlReq&&<div style={{marginBottom:8}}><div style={{color:'rgba(255,179,217,.4)',fontSize:'0.7rem',marginBottom:5}}>🔒 Unlocks at {lvlReq.emoji} {lvlReq.title}</div><div style={{height:4,background:'rgba(255,255,255,.05)',borderRadius:4,overflow:'hidden'}}><div style={{height:'100%',width:Math.min(100,((st.glowPoints/Math.max(1,lvlReq.threshold))*100))+'%',background:'linear-gradient(90deg,'+T.a1+'66,'+T.a2+'66)',borderRadius:4}}/></div></div>}{!locked&&!redeemed&&<div style={{marginBottom:8}}><div style={{height:4,background:'rgba(255,255,255,.05)',borderRadius:4,overflow:'hidden'}}><div style={{height:'100%',width:Math.min(100,(balance/r.cost)*100)+'%',background:'linear-gradient(90deg,'+ptColor+','+ptColor+'88)',borderRadius:4}}/></div>{!canAfford&&<div style={{color:'rgba(255,179,217,.3)',fontSize:'0.62rem',marginTop:3}}>{r.cost-balance} more {isGlow?'Glow':'Grind'} Pts needed</div>}</div>}{redeemed?<div style={{fontSize:'0.75rem',color:'rgba(255,179,217,.35)',fontStyle:'italic'}}>Redeemed ✓</div>:!locked&&<button onClick={function(){if(!canAfford)return;if(!window.confirm('Redeem?'))return;setSt(function(p){var nr2=p.rewards.map(function(x){return x.id===r.id?Object.assign({},x,{redeemed:true,redeemedAt:new Date().toISOString()}):x;});var ng=isGlow?Math.max(0,p.glowPoints-r.cost):p.glowPoints;var ngr=!isGlow?Math.max(0,p.grindPoints-r.cost):p.grindPoints;return Object.assign({},p,{rewards:nr2,glowPoints:ng,grindPoints:ngr});});}} style={{background:canAfford?'linear-gradient(135deg,'+T.a1+','+T.a2+')':'rgba(255,255,255,.06)',border:'none',borderRadius:20,color:canAfford?'#fff':'rgba(255,179,217,.25)',fontFamily:'Playfair Display,serif',fontSize:'0.78rem',fontWeight:'bold',padding:'7px 16px',cursor:canAfford?'pointer':'not-allowed',touchAction:'manipulation'}}>{canAfford?'Redeem 💖':'Not enough points'}</button>}</div></div></div>; })}</div>}
        </div>}
