...
async function loadFromSupabase() {
  try {
    const res = await fetch(SUPA_URL + '/rest/v1/glowup_data?id=eq.shared&select=data', {
...
async function saveToSupabase(data) {
  try {
    await fetch(SUPA_URL + '/rest/v1/glowup_data?id=eq.shared', {
      method: 'PATCH',
...
function loadData() { try { return JSON.parse(localStorage.getItem(SK)) || {}; } catch(e) { return {}; } }
function saveData(d) { localStorage.setItem(SK, JSON.stringify(d)); saveToSupabase(d); }

function pointsForDiff(diff) {
  if (diff === 'Hard') return 20;
  if (diff === 'Just Right') return 15;
  return 10;
}

function pickChallenges(seen, total, ovr, custom) {
  var today = new Date().toDateString(); var dow = new Date().getDay();
  var all = CHALLENGES.concat(custom || []);
  var pinned = all.filter(function(c) {
    var o = ovr[c.id] || {}; if (o.disabled) return false;
    return o.rt === 'daily' || (o.rt === 'days' && (o.days||[]).indexOf(dow) >= 0) || (o.rt === 'date' && o.date === today);
  }).slice(0,3);
  if (pinned.length >= 3) return pinned.slice(0,3);
  var lvl = getLvl(total).level; var pids = pinned.map(function(c){return c.id;});
...
export default function App() {
  ...
  var [st, setSt] = useState({
    ...
    herName: s.herName || 'Rebecca',
    reminderTime: s.reminderTime || '',
    points: s.points || 0,
    rewards: s.rewards || [],
    redeemedRewards: s.redeemedRewards || [],
    naughtyUnlockPoints: s.naughtyUnlockPoints || 100,
    ...
    wpexp: null, qpin: null, swapId: null,
    rewardForm: {emoji:'🎁',title:'',desc:'',cost:50}, rewardErr: '',
    showRewardRedeem: null,
  });

  useEffect(function() {
    saveData({ch:st.ch,lgd:st.lgd,total:st.total,seen:st.seen,hist:st.hist,streak:st.streak,lsd:st.lsd,custom:st.custom,ovr:st.ovr,cats:st.cats,earned:st.earned,msg:st.msg,reaction:st.reaction,reactions:st.reactions,theme:st.theme,wplan:st.wplan,pinSet:st.pinSet,pin:st.pin,herName:st.herName,reminderTime:st.reminderTime,points:st.points,rewards:st.rewards,redeemedRewards:st.redeemedRewards,naughtyUnlockPoints:st.naughtyUnlockPoints,lastActive:st.lastActive});
  }, [st.ch,st.lgd,st.total,st.seen,st.hist,st.streak,st.lsd,st.custom,st.ovr,st.cats,st.earned,st.msg,st.reaction,st.reactions,st.theme,st.wplan,st.pinSet,st.pin,st.herName,st.reminderTime,st.points,st.rewards,st.redeemedRewards,st.naughtyUnlockPoints,st.lastActive]);
  ...
  useEffect(function() {
    ...
    loadFromSupabase().then(function(remote) {
      if (remote && remote.total !== undefined) {
        setSt(function(p) {
          var merged = Object.assign({}, p, remote);
          if (!remote.lgd || remote.lgd !== todayStr || !remote.ch || remote.ch.length === 0) {
            merged.lgd = p.lgd;
            merged.ch = p.ch;
          }
          return merged;
  ...
  var lvi = getLvl(st.total); var nxt = getNext(st.total);
  ...
  var naughtyUnlocked = st.points >= st.naughtyUnlockPoints;

  function gen(base) {
    var b = base || st; var all = CHALLENGES.concat(b.custom);
    var plan = b.wplan[today]; var picked;
  ...
  function complete(uid) {
    setSt(function(p) {
      ...
      var pts = nd ? p.points + pointsForDiff(ch.diff) : Math.max(0, p.points - pointsForDiff(ch.diff));
      ...
      var ns = Object.assign({},p,{ch:nc,total:nt,seen:ns2,celebId:nd?uid:null,confetti:conf,lvlUp:lu,hist:Object.assign({},p.hist,{[today]:nc}),streak:str,lsd:lsd,allDone:an&&nd,cats:ncc,points:pts});
  ...
  function addReward() {
    var f = st.rewardForm;
    if (!f.title.trim()) { setSt(function(p){return Object.assign({},p,{rewardErr:'Add a title'});}); return; }
    var reward = {id:'r-'+Date.now(),emoji:f.emoji,title:f.title,desc:f.desc,cost:Number(f.cost),active:true};
    setSt(function(p){ return Object.assign({},p,{rewards:p.rewards.concat([reward]),rewardForm:{emoji:'🎁',title:'',desc:'',cost:50},rewardErr:''}); });
  }

  function redeemReward(reward) {
    if (st.points < reward.cost) return;
    setSt(function(p){
      return Object.assign({},p,{
        points: p.points - reward.cost,
        redeemedRewards: p.redeemedRewards.concat([{id:reward.id,title:reward.title,emoji:reward.emoji,date:new Date().toDateString(),pending:true}]),
        showRewardRedeem: null,
      });
    });
  }
    ...
    <div style={{minHeight:'100vh',background:T.bg,fontFamily:'Palatino Linotype,Palatino,Georgia,serif',position:'relative',overflow:'hidden'}}>
      ...
      {splash && (
        <div style={{position:'fixed',inset:0,background:T.bg,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',zIndex:999,animation:'splFade .5s 1.8s ease-out forwards'}}>
          ...
          <div style={{color:'rgba(255,179,217,.45)',fontStyle:'italic',marginTop:8,fontFamily:'Crimson Text,serif',fontSize:'1rem'}}>{st.herName} daily challenges</div>
        </div>
      )}
      ...
      {st.allDone && (
        <OVL onClose={function(){setSt(function(p){return Object.assign({},p,{allDone:false});});}}>
          <div style={Object.assign({},cardS,{padding:'44px 32px',textAlign:'center',animation:'lvlPop .6s ease-out',border:'1px solid '+T.a1+'55'})}>
            ...
            <div style={{marginTop:14,color:'#ffb347',fontFamily:'Playfair Display,serif',fontSize:'0.9rem'}}>+{pointsForDiff(null)} points earned today</div>
            {st.streak > 1 && <div style={{marginTop:6,color:'#ffb347',fontFamily:'Playfair Display,serif',fontSize:'0.9rem'}}>🔥 {st.streak} day streak!</div>}
          </div>
        </OVL>
      )}
      ...
      {st.showPin && (
        <OVL onClose={function(){setSt(function(p){return Object.assign({},p,{showPin:false});});}}>
          <div style={Object.assign({},cardS,{padding:'36px 24px',textAlign:'center',animation:'popIn .3s ease-out',border:'1px solid '+T.a1+'44'})}>
            ...
            <input type='number' pattern='[0-9]*' placeholder='0000' value={st.pi} onChange={function(e){setSt(function(p){return Object.assign({},p,{pi:e.target.value,pe:''});});}} onKeyDown={function(e){if(e.key==='Enter')submitPin();}} style={Object.assign({},inpS,{textAlign:'center',fontSize:'1.6rem',letterSpacing:'0.4em',marginBottom:12,MozAppearance:'textfield',WebkitAppearance:'none'})}/>
      ...
      {st.diffId && diffC && (
        <OVL onClose={function(){setSt(function(p){return Object.assign({},p,{diffId:null});});}}>
          <div style={Object.assign({},cardS,{padding:'24px',animation:'popIn .3s ease-out',border:'1px solid '+T.a1+'38',textAlign:'center'})}>
            ...
            <div style={{color:'#ffe8f5',fontFamily:'Playfair Display,serif',fontSize:'1rem',fontWeight:'bold',marginBottom:8}}>{diffC.emoji} {diffC.title}</div>
            <div style={{color:'rgba(255,179,217,.4)',fontSize:'0.72rem',marginBottom:18,fontFamily:'Crimson Text,serif'}}>Easy = 10pts, Just Right = 15pts, Hard = 20pts</div>
            <div style={{display:'flex',gap:8}}>
              {[['😊','Easy','10'],['💪','Just Right','15'],['🔥','Hard','20']].map(function(pair){
                return (
                  <button key={pair[1]} onClick={function(){setSt(function(p){return Object.assign({},p,{ch:p.ch.map(function(c){return c.uid===p.diffId?Object.assign({},c,{diff:pair[1]}):c;}),diffId:null});});}} style={{flex:1,padding:'10px 6px',borderRadius:10,border:'1px solid rgba(255,120,190,.2)',background:'transparent',color:'rgba(255,179,217,.5)',cursor:'pointer',fontFamily:'Crimson Text,serif',fontSize:'.82rem',touchAction:'manipulation'}}>
                    <div style={{fontSize:'1.4rem',marginBottom:2}}>{pair[0]}</div>
                    <div>{pair[1]}</div>
                    <div style={{fontSize:'0.65rem',color:T.a1,marginTop:2}}>+{pair[2]}pts</div>
                  </button>
                );
              })}
            </div>
      ...
      {st.showRewardRedeem && (
        <OVL onClose={function(){setSt(function(p){return Object.assign({},p,{showRewardRedeem:null});});}}>
          <div style={Object.assign({},cardS,{padding:'32px 24px',animation:'popIn .3s ease-out',border:'1px solid '+T.a1+'44',textAlign:'center'})}>
            <div style={{fontSize:'3rem',marginBottom:12}}>{st.showRewardRedeem.emoji}</div>
            <div style={{fontFamily:'Playfair Display,serif',fontWeight:'bold',fontSize:'1.2rem',color:'#ffe8f5',marginBottom:8}}>{st.showRewardRedeem.title}</div>
            {st.showRewardRedeem.desc && <div style={{color:'rgba(255,179,217,.5)',fontFamily:'Crimson Text,serif',fontSize:'0.88rem',marginBottom:16,lineHeight:1.5}}>{st.showRewardRedeem.desc}</div>}
            <div style={{color:T.a1,fontFamily:'Playfair Display,serif',fontWeight:'bold',fontSize:'1.1rem',marginBottom:20}}>{st.showRewardRedeem.cost} points</div>
            {st.points >= st.showRewardRedeem.cost ? (
              <button style={gbS} onClick={function(){redeemReward(st.showRewardRedeem);}}>Redeem Reward 🎁</button>
            ) : (
              <div style={{color:'rgba(255,179,217,.4)',fontFamily:'Crimson Text,serif',fontSize:'0.88rem'}}>You need {st.showRewardRedeem.cost - st.points} more points</div>
            )}
          </div>
        </OVL>
      )}
      ...
      <div style={{position:'relative',zIndex:1,maxWidth:520,margin:'0 auto',padding:'0 15px 80px'}}>
        ...
        <div style={{textAlign:'center',padding:'32px 0 16px'}}>
          ...
          <p style={{color:'rgba(255,179,217,.4)',fontStyle:'italic',margin:'6px 0 0',fontSize:'0.88rem',fontFamily:'Crimson Text,serif'}}>{st.herName} daily challenges</p>
          ...
          <div style={{display:'flex',alignItems:'center',justifyContent:'center',gap:8,marginTop:13,flexWrap:'wrap'}}>
            ...
            <div style={{display:'inline-flex',alignItems:'center',gap:6,background:'rgba(255,179,71,.08)',border:'1px solid rgba(255,179,71,.2)',borderRadius:50,padding:'9px 14px'}}>
              <span>⭐</span><div style={{color:'#ffb347',fontWeight:'bold',fontFamily:'Playfair Display,serif',fontSize:'0.86rem'}}>{st.points}pts</div>
            </div>
            {st.streak > 0 && (
              <div style={{display:'inline-flex',alignItems:'center',gap:6,background:'rgba(255,179,71,.08)',border:'1px solid rgba(255,179,71,.2)',borderRadius:50,padding:'9px 14px'}}>
        ...
        {st.view==='her' && (
          <div style={{animation:'up .4s ease-out'}}>
            <div style={{display:'flex',gap:7,marginBottom:14,overflowX:'auto',padding:'2px 0'}}>
              {['Today','Rewards','Stats','History','Messages'].map(function(t){
                return <button key={t} style={Object.assign({},sbS(st.htab===t),{flex:1,fontSize:'0.72rem',padding:'8px 10px'})} onClick={function(){setSt(function(p){return Object.assign({},p,{htab:t});});}}>{t}</button>;
              })}
            </div>
                    ...
                    return (
                      <div key={c.uid} style={Object.assign({},cardS,{padding:'18px',animation:'popIn .4s '+(i*.12)+'s ease-out both',border:c.done?'1px solid '+T.a1+'36':'1px solid rgba(255,120,190,.13)',background:c.done?T.a1+'07':'rgba(255,255,255,.035)',position:'relative',overflow:'hidden'})}>
                        ...
                        <div style={{display:'flex',alignItems:'flex-start',gap:12,paddingLeft:7}}>
                          <button style={cbS(c.done)} onClick={function(){complete(c.uid);}}>{c.done?'v':''}</button>
                          <div style={{flex:1,minWidth:0}}>
                            ...
                            {c.done && (
                              <div style={{display:'flex',gap:8,marginTop:10,flexWrap:'wrap'}}>
                                ...
                                <button onClick={function(){setSt(function(p){return Object.assign({},p,{diffId:c.uid});});}} style={{background:'transparent',border:'1px solid rgba(255,120,190,.22)',borderRadius:20,color:'rgba(255,179,217,.5)',cursor:'pointer',fontSize:'0.7rem',padding:'4px 12px',fontFamily:'Crimson Text,serif',touchAction:'manipulation'}}>{c.diff?(c.diff==='Easy'?'😊 ':c.diff==='Just Right'?'💪 ':'🔥 ')+c.diff+' (+'+(c.diff==='Hard'?20:c.diff==='Just Right'?15:10)+'pts)':'Rate for points'}</button>
                              </div>
                            )}
            ...
            {st.htab==='Rewards' && (
              <div style={{animation:'up .35s ease-out'}}>
                <div style={Object.assign({},cardS,{padding:'16px',marginBottom:14,textAlign:'center',border:'1px solid '+T.a1+'33'})}>
                  <div style={{fontSize:'2rem',marginBottom:6}}>⭐</div>
                  <div style={{fontFamily:'Playfair Display,serif',fontWeight:'bold',fontSize:'1.8rem',color:T.a1}}>{st.points}</div>
                  <div style={{color:'rgba(255,179,217,.4)',fontSize:'0.72rem',textTransform:'uppercase',letterSpacing:'0.1em',marginTop:2}}>Points Available</div>
                  {!naughtyUnlocked && (
                    <div style={{marginTop:10,padding:'8px 14px',background:'rgba(255,68,68,.08)',border:'1px solid rgba(255,68,68,.2)',borderRadius:12,fontSize:'0.76rem',color:'rgba(255,150,150,.6)',fontFamily:'Crimson Text,serif'}}>
                      😈 Naughty challenges unlock at {st.naughtyUnlockPoints} points - {Math.max(0,st.naughtyUnlockPoints-st.points)} to go
                    </div>
                  )}
                  {naughtyUnlocked && (
                    <div style={{marginTop:10,padding:'8px 14px',background:'rgba(255,68,68,.12)',border:'1px solid rgba(255,68,68,.3)',borderRadius:12,fontSize:'0.76rem',color:'rgba(255,150,150,.8)',fontFamily:'Playfair Display,serif',fontWeight:'bold'}}>
                      😈 Naughty challenges unlocked!
                    </div>
                  )}
                </div>

                {st.rewards.filter(function(r){return r.active;}).length === 0 ? (
                  <div style={{textAlign:'center',padding:'40px 20px',color:'rgba(255,179,217,.26)',fontStyle:'italic',fontFamily:'Crimson Text,serif'}}>No rewards set yet. Ask him to add some in Admin! 🎁</div>
                ) : (
                  <div style={{display:'flex',flexDirection:'column',gap:10}}>
                    {st.rewards.filter(function(r){return r.active;}).map(function(r){
                      var canAfford = st.points >= r.cost;
                      return (
                        <div key={r.id} style={Object.assign({},cardS,{padding:'16px',border:canAfford?'1px solid '+T.a1+'44':'1px solid rgba(255,120,190,.13)',opacity:canAfford?1:.7})}>
                          <div style={{display:'flex',alignItems:'center',gap:12}}>
                            <span style={{fontSize:'2rem'}}>{r.emoji}</span>
                            <div style={{flex:1}}>
                              <div style={{color:'#ffe8f5',fontFamily:'Playfair Display,serif',fontWeight:'bold',fontSize:'0.92rem'}}>{r.title}</div>
                              {r.desc && <div style={{color:'rgba(255,179,217,.5)',fontFamily:'Crimson Text,serif',fontSize:'0.8rem',marginTop:2,lineHeight:1.4}}>{r.desc}</div>}
                              <div style={{color:canAfford?T.a1:'rgba(255,179,217,.3)',fontSize:'0.72rem',marginTop:4,fontWeight:'bold'}}>{r.cost} points</div>
                            </div>
                            <button onClick={function(){setSt(function(p){return Object.assign({},p,{showRewardRedeem:r});});}} style={{background:canAfford?grad:'rgba(255,255,255,.05)',border:'none',borderRadius:20,color:canAfford?'#fff':'rgba(255,179,217,.3)',cursor:'pointer',fontSize:'0.76rem',padding:'8px 14px',fontFamily:'Playfair Display,serif',fontWeight:'bold',touchAction:'manipulation',whiteSpace:'nowrap'}}>{canAfford?'Redeem':'Need more'}</button>
                          </div>
                        </div>
                      );
                    })}
                ...
                {st.redeemedRewards.length > 0 && (
                  <div style={{marginTop:20}}>
                    <div style={{color:'rgba(255,179,217,.32)',fontSize:'0.64rem',textTransform:'uppercase',letterSpacing:'0.14em',marginBottom:10}}>Redeemed Rewards</div>
                    {st.redeemedRewards.map(function(r,i){
                      return (
                        <div key={i} style={Object.assign({},cardS,{padding:'12px 14px',marginBottom:8,display:'flex',alignItems:'center',gap:10,opacity:r.pending?1:.5})}>
                          <span style={{fontSize:'1.4rem'}}>{r.emoji}</span>
                          <div style={{flex:1}}>
                            <div style={{color:'#ffe8f5',fontFamily:'Playfair Display,serif',fontSize:'0.84rem'}}>{r.title}</div>
                            <div style={{color:'rgba(255,179,217,.38)',fontSize:'0.68rem',marginTop:2}}>{r.date} {r.pending?' - pending':'- claimed'}</div>
                          </div>
                          {r.pending && <div style={{color:T.a1,fontSize:'0.7rem',fontFamily:'Playfair Display,serif'}}>Pending</div>}
                        </div>
                      );
                    })}
                  </div>
            ...
            {st.htab==='Stats' && (
              <div style={{animation:'up .35s ease-out'}}>
                <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10,marginBottom:14}}>
                  {[{e:'✅',v:st.total,l:'Total Done'},{e:'🔥',v:st.streak,l:'Streak'},{e:'⭐',v:st.points,l:'Points'},{e:lvi.emoji,v:lvi.title,l:'Level'}].map(function(s,i){
                    return (
                  ...
                  return (
                    <div key={date} style={Object.assign({},cardS,{padding:'13px 16px',marginBottom:10,border:pf?'1px solid '+T.a1+'30':'1px solid rgba(255,120,190,.11)'})}>
                      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:7}}>
                        ...
                        <span style={{fontSize:'0.72rem',color:pf?T.a1:'rgba(255,179,217,.3)',fontFamily:'Playfair Display,serif'}}>{dd}/{dc.length}{pf?' crown':''}</span>
                      </div>
                      {dc.map(function(c,i){
                        return (
                          <div key={i}>
                            <div style={{display:'flex',alignItems:'center',gap:8,marginTop:5,opacity:c.done?1:.32}}>
                              <span style={{fontSize:'0.78rem'}}>{c.done?'done':'skip'}</span>
            ...
            {st.htab==='Messages' && (
              <div style={{animation:'up .35s ease-out'}}>
                <div style={{color:'rgba(255,179,217,.32)',fontSize:'0.64rem',textTransform:'uppercase',letterSpacing:'0.14em',marginBottom:12,textAlign:'center'}}>Messages from Him</div>
                {st.reactions.length === 0 ? (
                  <div style={{textAlign:'center',padding:'50px 20px',color:'rgba(255,179,217,.26)',fontStyle:'italic',fontFamily:'Crimson Text,serif'}}>Messages he sends you will appear here</div>
                ) : [...st.reactions].reverse().map(function(r,i){
                  return (
                    <div key={i} style={Object.assign({},cardS,{padding:'14px 16px',marginBottom:10,border:'1px solid '+T.a1+'28',background:T.a1+'06'})}>
                      <div style={{display:'flex',alignItems:'flex-start',gap:10}}>
                        <span style={{fontSize:'1.4rem'}}>💌</span>
                        <div style={{flex:1}}>
                          <div style={{color:'#ffe8f5',fontFamily:'Crimson Text,serif',fontSize:'0.92rem',lineHeight:1.55}}>{r.text}</div>
                          <div style={{color:'rgba(255,179,217,.3)',fontSize:'0.66rem',marginTop:5}}>{r.date}</div>
                        </div>
                      </div>
                    </div>
                  );
        ...
        {st.view==='admin' && st.unlocked && (
          <div style={{animation:'up .4s ease-out'}}>
            <div style={{display:'flex',gap:5,marginBottom:14,overflowX:'auto',padding:'2px 0'}}>
              {['Today','Rewards','Challenges','Planner','Add New','Bulk','Settings'].map(function(t){
                return <button key={t} style={Object.assign({},sbS(st.atab===t),{flex:1,minWidth:'fit-content',padding:'8px 7px',fontSize:'0.68rem'})} onClick={function(){setSt(function(p){return Object.assign({},p,{atab:t});});}}>{t}</button>;
              })}
            </div>

            {st.atab==='Today' && (
              <div>
                <div style={Object.assign({},cardS,{padding:'14px',marginBottom:12,border:'1px solid '+T.a1+'33'})}>
                  <div style={{color:'rgba(255,179,217,.4)',fontSize:'0.62rem',textTransform:'uppercase',letterSpacing:'0.1em',marginBottom:10}}>Live Stats</div>
                  <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:8}}>
                    {[{l:'Level',v:lvi.title,e:lvi.emoji},{l:'Streak',v:st.streak+'d',e:'🔥'},{l:'Points',v:st.points,e:'⭐'},{l:'Done Today',v:doneT+'/'+st.ch.length,e:'✅'},{l:'Total',v:st.total,e:'💅'},{l:'Last Active',v:st.lastActive?new Date(st.lastActive).toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'}):'never',e:'📱'}].map(function(s,i){
                      return (
                        <div key={i} style={Object.assign({},cardS,{padding:'10px 6px',textAlign:'center'})}>
                          <div style={{fontSize:'1.1rem',marginBottom:3}}>{s.e}</div>
                          <div style={{color:'#ffb3d9',fontFamily:'Playfair Display,serif',fontWeight:'bold',fontSize:'0.74rem'}}>{s.v}</div>
                          <div style={{color:'rgba(255,179,217,.3)',fontSize:'0.55rem',marginTop:2,textTransform:'uppercase',letterSpacing:'0.07em'}}>{s.l}</div>
                        </div>
                      );
                    })}
                  </div>
                  <button onClick={function(){loadFromSupabase().then(function(r){if(r)setSt(function(p){return Object.assign({},p,r);});});}} style={Object.assign({},sbS(false),{width:'100%',marginTop:10,fontSize:'0.74rem',padding:'8px'})}>Refresh Stats</button>
                </div>
                ...
                <div style={Object.assign({},cardS,{padding:'14px',marginBottom:11})}>
                  ...
                  <button style={gbS} onClick={function(){
                    if(!st.qpin)return;
                    var ch=allCh.find(function(c){return c.id===st.qpin;}); if(!ch)return;
                ...
                <div style={Object.assign({},cardS,{padding:'14px',marginBottom:11})}>
                  <div style={lblS}>Daily message for {st.herName}</div>
                ...
                <div style={Object.assign({},cardS,{padding:'14px',marginBottom:11})}>
                  ...
                  <textarea rows={2} placeholder='A note she will see and keep...' value={st.reactd} onChange={function(e){setSt(function(p){return Object.assign({},p,{reactd:e.target.value});});}} style={Object.assign({},inpS,{resize:'none',lineHeight:1.5,marginBottom:9})}/>
                  <button style={gbS} onClick={function(){setSt(function(p){
                    if(!p.reactd.trim())return p;
                    var newReaction = {text:p.reactd,date:new Date().toDateString()};
                    return Object.assign({},p,{reaction:p.reactd,reactions:p.reactions.concat([newReaction]),reactd:''});
                  });}}>Send 💖</button>
                </div>
                ...
                <div style={Object.assign({},cardS,{padding:'14px',marginBottom:11})}>
                  <div style={lblS}>Today's 3 Challenges</div>
                  {st.ch.map(function(c,i){
                    var cc=CAT_COLOR[c.category]||'#ff6eb4';
                    return (
                      <div key={c.uid} style={{marginBottom:10,padding:'10px 12px',background:'rgba(255,255,255,.03)',borderRadius:12,border:'1px solid rgba(255,120,190,.15)'}}>
                        <div style={{display:'flex',alignItems:'center',gap:8}}>
                          <span style={{fontSize:'0.95rem'}}>{c.emoji}</span>
                          <div style={{flex:1,minWidth:0}}>
                            <div style={{color:'#ffe8f5',fontFamily:'Playfair Display,serif',fontSize:'0.8rem',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{c.title}</div>
                            <div style={{color:cc,fontSize:'0.6rem',marginTop:2}}>{CAT_ICON[c.category]} {c.category} · {c.done?'done':'pending'}</div>
                          </div>
                          <button onClick={function(){setSt(function(p){return Object.assign({},p,{swapId:c.uid});});}} style={{background:'linear-gradient(135deg,'+T.a1+','+T.a2+')',border:'none',borderRadius:20,color:'#fff',cursor:'pointer',fontSize:'0.7rem',padding:'5px 12px',fontFamily:'Playfair Display,serif',fontWeight:'bold',touchAction:'manipulation',whiteSpace:'nowrap'}}>Swap</button>
                        </div>
                        {st.swapId===c.uid && (
                          <div style={{marginTop:10}}>
                            <div style={{color:'rgba(255,179,217,.46)',fontSize:'0.66rem',textTransform:'uppercase',letterSpacing:'0.1em',marginBottom:6}}>Pick a replacement</div>
                            <select defaultValue='' onChange={function(e){
                              if(!e.target.value)return;
                              var newCh=allCh.find(function(x){return x.id===e.target.value;});
                              if(!newCh)return;
                              setSt(function(p){
                                var updated=p.ch.map(function(x){
                                  return x.uid===p.swapId?Object.assign({},newCh,{uid:x.uid,done:false,note:'',diff:null}):x;
                                });
                                return Object.assign({},p,{ch:updated,lgd:today,swapId:null});
                              });
                            }} style={Object.assign({},inpS,{fontSize:'0.82rem'})}>
                              <option value=''>Choose a challenge...</option>
                              {allCh.filter(function(x){return !st.ch.find(function(y){return y.id===x.id&&y.uid!==c.uid;});}).map(function(x){return <option key={x.id} value={x.id}>{x.emoji} {x.title} ({x.category})</option>;})}
                            </select>
                            <button onClick={function(){setSt(function(p){return Object.assign({},p,{swapId:null});});}} style={{background:'transparent',border:'none',color:'rgba(255,179,217,.4)',cursor:'pointer',fontSize:'0.72rem',marginTop:6,padding:0,fontFamily:'Crimson Text,serif',touchAction:'manipulation'}}>Cancel</button>
                          </div>
                        )}
                      </div>
                    );
            ...
            {st.atab==='Rewards' && (
              <div>
                <p style={{color:'rgba(255,179,217,.4)',fontSize:'0.78rem',fontFamily:'Crimson Text,serif',marginBottom:13,lineHeight:1.5}}>Set rewards she can redeem with her points. She earns 10, 15, or 20 points per challenge based on difficulty.</p>

                <div style={Object.assign({},cardS,{padding:'14px',marginBottom:14})}>
                  <div style={{color:T.a1,fontFamily:'Playfair Display,serif',fontSize:'0.88rem',fontWeight:'bold',marginBottom:12}}>Add New Reward</div>
                  <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:9,marginBottom:10}}>
                    <div>
                      <div style={lblS}>Title</div>
                      <input value={st.rewardForm.title} placeholder='e.g. Date night' onChange={function(e){setSt(function(p){return Object.assign({},p,{rewardForm:Object.assign({},p.rewardForm,{title:e.target.value})});});}} style={inpS}/>
                    </div>
                    <div>
                      <div style={lblS}>Emoji</div>
                      <input value={st.rewardForm.emoji} onChange={function(e){setSt(function(p){return Object.assign({},p,{rewardForm:Object.assign({},p.rewardForm,{emoji:e.target.value})});});}} style={Object.assign({},inpS,{textAlign:'center',fontSize:'1.3rem'})} maxLength={2}/>
                    </div>
                  </div>
                  <div style={{marginBottom:10}}>
                    <div style={lblS}>Description (optional)</div>
                    <input value={st.rewardForm.desc} placeholder='e.g. Dinner and a film of your choice' onChange={function(e){setSt(function(p){return Object.assign({},p,{rewardForm:Object.assign({},p.rewardForm,{desc:e.target.value})});});}} style={inpS}/>
                  </div>
                  <div style={{marginBottom:12}}>
                    <div style={lblS}>Point cost</div>
                    <input type='number' value={st.rewardForm.cost} onChange={function(e){setSt(function(p){return Object.assign({},p,{rewardForm:Object.assign({},p.rewardForm,{cost:e.target.value})});});}} style={inpS}/>
                  </div>
                  {st.rewardErr && <p style={{color:'#ff3d9a',fontSize:'0.82rem',marginBottom:10}}>{st.rewardErr}</p>}
                  <button style={gbS} onClick={addReward}>Add Reward 🎁</button>
                </div>

                <div style={{marginBottom:14}}>
                  <div style={lblS}>Naughty Challenge Unlock Point Threshold</div>
                  <div style={{display:'flex',gap:9,alignItems:'center'}}>
                    <input type='number' value={st.naughtyUnlockPoints} onChange={function(e){setSt(function(p){return Object.assign({},p,{naughtyUnlockPoints:Number(e.target.value)});});}} style={Object.assign({},inpS,{flex:1})}/>
                    <div style={{color:naughtyUnlocked?'#ff4444':'rgba(255,179,217,.4)',fontSize:'0.74rem',whiteSpace:'nowrap',fontFamily:'Crimson Text,serif'}}>{naughtyUnlocked?'Unlocked':'Locked'}</div>
                  </div>
                  <div style={{color:'rgba(255,179,217,.3)',fontSize:'0.7rem',marginTop:5,fontFamily:'Crimson Text,serif'}}>She needs this many points before she can see Naughty challenges</div>
                </div>

                {st.rewards.length > 0 && (
                  <div>
                    <div style={{color:'rgba(255,179,217,.32)',fontSize:'0.64rem',textTransform:'uppercase',letterSpacing:'0.14em',marginBottom:10}}>Your Rewards</div>
                    {st.rewards.map(function(r,i){
                      return (
                        <div key={r.id} style={Object.assign({},cardS,{padding:'12px 14px',marginBottom:8,display:'flex',alignItems:'center',gap:10})}>
                          <span style={{fontSize:'1.5rem'}}>{r.emoji}</span>
                          <div style={{flex:1,minWidth:0}}>
                            <div style={{color:'#ffe8f5',fontFamily:'Playfair Display,serif',fontSize:'0.86rem',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{r.title}</div>
                            <div style={{color:T.a1,fontSize:'0.7rem',marginTop:2}}>{r.cost} points</div>
                          </div>
                          <button onClick={function(){setSt(function(p){var rr=p.rewards.slice();rr[i]=Object.assign({},rr[i],{active:!rr[i].active});return Object.assign({},p,{rewards:rr});});}} style={{background:'transparent',border:'1px solid rgba(255,120,190,.22)',borderRadius:20,color:'rgba(255,179,217,.5)',cursor:'pointer',fontSize:'0.7rem',padding:'4px 10px',touchAction:'manipulation'}}>{r.active?'Hide':'Show'}</button>
                          <button onClick={function(){setSt(function(p){return Object.assign({},p,{rewards:p.rewards.filter(function(x){return x.id!==r.id;})});});}} style={{background:'transparent',border:'none',color:'rgba(255,100,100,.38)',cursor:'pointer',fontSize:'0.9rem',touchAction:'manipulation'}}>x</button>
                        </div>
                      );
                    })}
                  </div>
                ...
                {st.redeemedRewards.filter(function(r){return r.pending;}).length > 0 && (
                  <div style={{marginTop:16}}>
                    <div style={{color:'rgba(255,179,217,.32)',fontSize:'0.64rem',textTransform:'uppercase',letterSpacing:'0.14em',marginBottom:10}}>Pending Claims</div>
                    {st.redeemedRewards.filter(function(r){return r.pending;}).map(function(r,i){
                      return (
                        <div key={i} style={Object.assign({},cardS,{padding:'12px 14px',marginBottom:8,display:'flex',alignItems:'center',gap:10,border:'1px solid '+T.a1+'33'})}>
                          <span style={{fontSize:'1.4rem'}}>{r.emoji}</span>
                          <div style={{flex:1}}>
                            <div style={{color:'#ffe8f5',fontFamily:'Playfair Display,serif',fontSize:'0.84rem'}}>{r.title}</div>
                            <div style={{color:'rgba(255,179,217,.38)',fontSize:'0.68rem',marginTop:2}}>Claimed {r.date}</div>
                          </div>
                          <button onClick={function(){setSt(function(p){var rr=p.redeemedRewards.map(function(x,j){return j===i?Object.assign({},x,{pending:false}):x;});return Object.assign({},p,{redeemedRewards:rr});});}} style={Object.assign({},gbS,{width:'auto',padding:'8px 14px',fontSize:'0.72rem'})}>Mark Done</button>
                        </div>
                      );
                    })}
                  </div>
              ...
              <div>
                ...
                <div style={{display:'flex',flexDirection:'column',gap:7}}>
                  {filt.map(function(c){
                    ...
                    var isNaughty = c.category === 'Naughty';
                    return (
                        ...
                        <div style={{display:'flex',alignItems:'center',gap:8}}>
                          ...
                          <div style={{flex:1,minWidth:0}}>
                            ...
                            <div style={{display:'flex',alignItems:'center',gap:5,marginTop:2,flexWrap:'wrap'}}>
                              ...
                              {isNaughty && <span style={{color:naughtyUnlocked?'#ff4444':'rgba(255,120,190,.3)',fontSize:'0.56rem'}}>{naughtyUnlocked?'unlocked':'locked'}</span>}
                              {sched && <span style={{color:T.a1,fontSize:'0.56rem'}}>pinned {ov.rt==='daily'?'daily':ov.rt==='days'?(ov.days||[]).map(function(d){return DAYS[d];}).join(', '):ov.date||''}</span>}
                            </div>
                          </div>
                          <button onClick={function(){setSt(function(p){return Object.assign({},p,{schedId:c.id});});}} style={{background:'transparent',border:'1px solid rgba(255,120,190,.18)',borderRadius:8,color:sched?T.a1:'rgba(255,179,217,.34)',cursor:'pointer',fontSize:'0.66rem',padding:'4px 8px',touchAction:'manipulation'}}>pin</button>
            ...
            {st.atab==='Settings' && (
              <div>
                <div style={Object.assign({},cardS,{padding:'14px',marginBottom:11})}>
                  <div style={lblS}>Her Name</div>
                  <input value={st.herName} onChange={function(e){setSt(function(p){return Object.assign({},p,{herName:e.target.value});});}} style={inpS} placeholder='Rebecca'/>
                </div>

                <div style={Object.assign({},cardS,{padding:'14px',marginBottom:11})}>
                  <div style={lblS}>Daily Reminder Time</div>
                  <p style={{color:'rgba(255,179,217,.38)',fontSize:'0.74rem',fontFamily:'Crimson Text,serif',marginBottom:9,lineHeight:1.4}}>She will get a notification at this time each day if she has the app open.</p>
                  <input type='time' value={st.reminderTime} onChange={function(e){setSt(function(p){return Object.assign({},p,{reminderTime:e.target.value});});}} style={Object.assign({},inpS,{colorScheme:'dark',marginBottom:9})}/>
                  {st.reminderTime && <p style={{color:T.a1+'cc',fontSize:'0.74rem'}}>Reminder set for {st.reminderTime}</p>}
                </div>
