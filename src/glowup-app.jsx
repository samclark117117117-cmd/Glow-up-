...
const CATS = ['Beauty','Style','Glam','Confidence','Barbie','Devotion','Self-Care','Dare','Flirty','Naughty'];
const CAT_COLOR = {Beauty:'#ff6eb4',Style:'#c77dff',Glam:'#ffb347',Confidence:'#ff4db8',Barbie:'#ff69b4',Devotion:'#ff9bd2','Self-Care':'#90caf9',Dare:'#ff3d9a',Flirty:'#ff80cc',Naughty:'#ff4444'};
const CAT_ICON = {Beauty:'💄',Style:'👗',Glam:'✨',Confidence:'🦋',Barbie:'🩷',Devotion:'🩷','Self-Care':'💆',Dare:'📸',Flirty:'💋',Naughty:'😈'};
const CONF_EMOJI = ['💖','✨','🌸','💅','👑','🦋','💗','⭐','🎀','💎','🌟','🩷'];
...
const CHALLENGES = [
  ...
  // ── CONFIDENCE (25) ─────────────────────────────────────────────────────────
  {id:'c01',emoji:'🦋',title:'Five Things Out Loud',description:'Stand in front of the mirror for 5 minutes. Find five specific things you genuinely love about what you see — not personality traits, physical things. Say each one out loud clearly. Not in your head. Out loud.',category:'Confidence',level:1,ctx:'any'},
  {id:'c02',emoji:'🦋',title:'Wear It Unbuttoned',description:'Whatever you would normally button up or cover — do not. The extra button stays open all day. You do not adjust it once.',category:'Confidence',level:2,ctx:'out'},
  {id:'c03',emoji:'🦋',title:'Hold Eye Contact First',description:'Every single interaction today — cashier, colleague, anyone — you hold eye contact until they look away first. Every single one. You do not flinch.',category:'Confidence',level:2,ctx:'out'},
  {id:'c04',emoji:'🦋',title:'Send Him the Mirror Photo',description:'Take a full-length mirror photo right now exactly as you are. Send it to Sam with no filter, no caption, no apology. Just the photo.',category:'Confidence',level:2,ctx:'any'},
  {id:'c05',emoji:'🦋',title:'The Slow Walk',description:'Every time you walk anywhere today you slow down by 30%. No rushing, no head down, no phone in hand. You walk like you have nowhere to be and everywhere to be at once.',category:'Confidence',level:2,ctx:'out'},
  {id:'c06',emoji:'🦋',title:'Ask for the Upgrade',description:'At the first opportunity today — coffee shop, restaurant, anywhere — ask for something better than what you ordered. Upgrade the size, ask for the table you actually want, request something off menu. Ask directly with no apology.',category:'Confidence',level:3,ctx:'out'},
  {id:'c07',emoji:'🦋',title:'Gym Crop Top',description:'Train today in a crop top or sports bra. Nothing over the top. Just your body doing the work, visible. You do not think about it once you start.',category:'Confidence',level:2,ctx:'gym'},
  {id:'c08',emoji:'🦋',title:'Post Without Checking',description:'Take a photo, post it, put your phone down. You are not allowed to check likes or comments for two hours. It exists whether anyone sees it or not.',category:'Confidence',level:3,ctx:'any'},
  {id:'c09',emoji:'🦋',title:'Disagree Out Loud',description:'The next time someone says something you disagree with today, say so. Calmly, clearly, without softening it into a question. One sentence. You do not need their agreement.',category:'Confidence',level:3,ctx:'out'},
  {id:'c10',emoji:'🦋',title:'Body Oil in Public',description:'Apply body shimmer oil to your arms and legs before you go out. Not for a special occasion. For a Tuesday. Because you feel like it.',category:'Confidence',level:2,ctx:'out'},
  {id:'c11',emoji:'🦋',title:'Take Up the Space',description:'Wherever you sit today — sofa, chair, train seat — spread out fully. Bag beside you not on your lap. Legs uncrossed. Arms resting wide. You are not making yourself small for anyone.',category:'Confidence',level:2,ctx:'any'},
  {id:'c12',emoji:'🦋',title:'Tell Him What You Want',description:'Think of one specific thing you want Sam to do or say. Message him and ask for it directly. Not hinting. Not hoping he figures it out. Just ask.',category:'Confidence',level:3,ctx:'any'},
  {id:'c13',emoji:'🦋',title:'Stop the Apology',description:'Every time you catch yourself about to say sorry today for something that is not your fault — stop. Replace it with nothing or with a statement. Count how many times you catch it and tell Sam tonight.',category:'Confidence',level:3,ctx:'any'},
  {id:'c14',emoji:'🦋',title:'The Lingerie Rule',description:'Wear your nicest matching underwear today under the most ordinary outfit you own. Nobody will know. You will know. That is the entire point.',category:'Confidence',level:1,ctx:'any'},
  {id:'c15',emoji:'🦋',title:'One Hour Completely Alone',description:'One hour today with no phone, no music, no input. Just you in a room or on a walk. Sit with your own thoughts without filling the silence. Notice what comes up.',category:'Confidence',level:3,ctx:'any'},
  {id:'c16',emoji:'🦋',title:'Compliment a Stranger',description:'Give one genuine compliment to a stranger today — a woman whose bag you like, someone with great hair, anything real. Make eye contact, say it clearly, do not rush away.',category:'Confidence',level:3,ctx:'out'},
  {id:'c17',emoji:'🦋',title:'Wear the Short One',description:'The skirt or dress that feels too short for where you are going today — wear it anyway. You adjust it zero times.',category:'Confidence',level:3,ctx:'out'},
  {id:'c18',emoji:'🦋',title:'Say No Without a Reason',description:'The next time someone asks you to do something you do not want to do today — say no. No explanation, no excuse, no softening. Just no. Watch how the world does not end.',category:'Confidence',level:4,ctx:'any'},
  {id:'c19',emoji:'🦋',title:'Eat Alone and Enjoy It',description:'Have one meal today completely alone — no phone on the table, no scrolling, no distractions. Just you eating food and being present. Order exactly what you want.',category:'Confidence',level:3,ctx:'out'},
  {id:'c20',emoji:'🦋',title:'The Proof Photo',description:'Get fully ready — hair, makeup, outfit, the full thing — and take photos until you have one where you think you look genuinely incredible. Send it to Sam. Keep the photo.',category:'Confidence',level:2,ctx:'any'},
  {id:'c21',emoji:'🦋',title:'Introduce Yourself First',description:'In the first group situation today — meeting, social, anywhere — introduce yourself before anyone asks. Name, confident, done. You do not wait to be noticed.',category:'Confidence',level:3,ctx:'out'},
  {id:'c22',emoji:'🦋',title:'Mirror Workout',description:'Do your gym session or home workout in front of a mirror today. Watch yourself. Do not look away. You are building something and you are allowed to watch it being built.',category:'Confidence',level:3,ctx:'gym'},
  {id:'c23',emoji:'🦋',title:'Document the Progress',description:'Take photos of yourself today specifically to compare to older photos. Front and side. Same outfit if possible. Look at what is changing. Send them to Sam.',category:'Confidence',level:2,ctx:'any'},
  {id:'c24',emoji:'🦋',title:'The Difficult Conversation',description:'There is something you have been avoiding saying or asking. Say it today. One clear message, one phone call, whatever it takes. Do not end the day without saying the thing.',category:'Confidence',level:4,ctx:'any'},
  {id:'c25',emoji:'🦋',title:'Own the Compliment',description:'Every time someone compliments you today — say thank you and nothing else. No "oh it is nothing," no deflecting, no self-deprecation. Just thank you. Let it land.',category:'Confidence',level:2,ctx:'any'},
    // ── DARE (18) ────────────────────────────────────────────────────────────────
  ...
  // ── BARBIE (20) ──────────────────────────────────────────────────────────────
  {id:'bb01',emoji:'🩷',title:'All Pink Today',description:'Head to toe pink today. Different shades, different textures, whatever you have — but all pink. No exceptions, no neutral basics to break it up. Committed and fully in it.',category:'Barbie',level:1,ctx:'out'},
  {id:'bb02',emoji:'🩷',title:'Hot Pink Lip All Day',description:'Hot pink lip from the moment you leave the house until the moment you get home. Reapply every two hours without fail. Not nude, not berry — hot pink.',category:'Barbie',level:1,ctx:'out'},
  {id:'bb03',emoji:'🩷',title:'The Big Hair Day',description:'Hair must be big today. Blowdry with volume, use a round brush, backcomb if you need to. Bouncy, full, feminine. No flat hair, no bun, no ponytail. Big hair all day.',hint:'For maximum volume, blowdry upside down first to lift the roots. Use a round brush to curl each section under as you dry it. Finish with a few spritzes of volumising spray at the roots and scrunch upward. Backcomb lightly at the crown.',category:'Barbie',level:2,ctx:'any'},
  {id:'bb04',emoji:'🩷',title:'Pink Nails Today',description:'If your nails are not already pink, they need to be by the end of today. Hot pink, baby pink, neon pink, French with a pink base — your choice. But pink.',category:'Barbie',level:1,ctx:'any'},
  {id:'bb05',emoji:'🩷',title:'Bow in Your Hair',description:'A bow today — hair bow, bow headband, bow clip. Big enough to be noticed. Wear it with full confidence like it is the most normal thing in the world.',category:'Barbie',level:1,ctx:'any'},
  {id:'bb06',emoji:'🩷',title:'Gloss Over Everything',description:'Lip gloss today — over your lip liner, over your lipstick, alone, whatever. But you are glossy all day. Reapply every time you eat or drink. Catch your reflection and appreciate it.',category:'Barbie',level:1,ctx:'any'},
  {id:'bb07',emoji:'🩷',title:'The Barbie Outfit',description:'The most feminine thing in your wardrobe — the most pink, the most pretty, the most extra. Wear it today for the most ordinary task on your list. The supermarket is your runway.',category:'Barbie',level:2,ctx:'out'},
  {id:'bb08',emoji:'🩷',title:'Pink Blush Moment',description:'Apply your blush today higher and more generously than usual — tops of the cheekbones, a little on the nose, even light on the eyelids. The flushed doll look. Own it completely.',hint:'Apply blush to the tops of your cheekbones rather than the apples. Blend upward toward your temples. Then take a tiny amount on a small brush and dust lightly across the nose bridge. A touch on the eyelids ties it all together. The key is blending so it looks flushed not painted.',category:'Barbie',level:2,ctx:'any'},
  {id:'bb09',emoji:'🩷',title:'Matching Pink Set',description:'Matching set today — top and bottom in the same pink. Co-ord, matching tracksuit, two-piece outfit. Everything intentional and coordinated.',category:'Barbie',level:2,ctx:'out'},
  {id:'bb10',emoji:'🩷',title:'Send Him a Barbie Photo',description:'Get fully ready in your most pink, most feminine, most Barbie outfit and send Sam a photo with no caption. Just the photo. Let him see exactly what you look like today.',category:'Barbie',level:2,ctx:'any'},
  {id:'bb11',emoji:'🩷',title:'Pink Accessories Stack',description:'Stack every pink accessory you own today — rings, bracelets, necklaces, hair clips, bag. More is more. The goal is unmistakably intentional and completely committed.',category:'Barbie',level:2,ctx:'out'},
  {id:'bb12',emoji:'🩷',title:'Heels With Everything',description:'Heels today with whatever you are wearing. Even if it does not make obvious sense. Even for ordinary tasks. The most feminine shoes you own and you wear them all day.',category:'Barbie',level:3,ctx:'out'},
  {id:'bb13',emoji:'🩷',title:'Curled and Set',description:'Curls today — properly done, set, held. Big bouncy Barbie curls. Take the time to do them properly and spray them to last all day. Do not let them drop.',hint:'Section your hair and curl away from your face for a more open look. After curling each section do not brush it out — let it cool completely as a curl, then lightly separate with your fingers only. Spray with strong hold hairspray before separating for curls that last all day.',category:'Barbie',level:2,ctx:'any'},
  {id:'bb14',emoji:'🩷',title:'The Satin Moment',description:'Wear something satin or silky today — a satin skirt, satin top, satin slip dress. The texture is everything. Satin catches the light and moves in a way that nothing else does.',category:'Barbie',level:2,ctx:'out'},
  {id:'bb15',emoji:'🩷',title:'Pink Gym Look',description:'All pink gym outfit today. Matching set if you have it. You are training but you look incredible doing it. Send Sam a gym selfie before you leave.',category:'Barbie',level:2,ctx:'gym'},
  {id:'bb16',emoji:'🩷',title:'Perfume Cloud',description:'Apply your most feminine, most sweet perfume today and reapply every three hours. You leave a trail everywhere you go. People should notice you have been somewhere after you have left.',category:'Barbie',level:1,ctx:'any'},
  {id:'bb17',emoji:'🩷',title:'Full Barbie Face',description:'Pink eyeshadow, pink blush, pink gloss or lipstick. The full pink makeup look today — coordinated, deliberate, completely feminine. Go out in it and act like it is completely normal.',hint:'Baby pink shimmer on the lid, slightly deeper pink in the crease. High pink blush on the cheekbones. Pink lip liner slightly overdrawn then filled with a matching pink gloss. Keep the rest of the face clean and fresh so the pink tones are the focus.',category:'Barbie',level:3,ctx:'out'},
  {id:'bb18',emoji:'🩷',title:'Mini Skirt and Heels',description:'Mini skirt and heels today — no tights, no layers over the top, no covering up. Wear it for something completely ordinary and walk like you are supposed to look exactly like this.',category:'Barbie',level:3,ctx:'out'},
  {id:'bb19',emoji:'🩷',title:'The Pink Haul',description:'Buy one new pink piece today — clothing, accessory, beauty product, anything. Something that makes your heart do the thing when you look at it. Send Sam a photo of what you bought.',category:'Barbie',level:3,ctx:'out'},
  {id:'bb20',emoji:'🩷',title:'Document the Barbie Day',description:'Photo documentation of your look today — morning, midday, evening. Three photos showing the full Barbie day. Send all three to Sam at the end of the night with a rundown of how you felt in it.',category:'Barbie',level:2,ctx:'any'},
    {id:'da19',emoji:'📸',title:'Weekly Signal Photo',description:'This week send Sam one photo on Signal that you would never post anywhere else. Something just for him. Could be a gym progress photo, a full outfit, something more personal — your choice. Send it before Sunday.',category:'Dare',level:2,ctx:'any'},
  {id:'da20',emoji:'📸',title:'Progress Documentation',description:'Take front and side progress photos today specifically to track your transformation. Same spot, same lighting if possible. Send them to Sam on Signal with a voice note telling him what you notice changing.',category:'Dare',level:2,ctx:'any'},
    // ── SELF-CARE (18) ───────────────────────────────────────────────────────────
  {id:'e01',emoji:'💆',title:'The Full Gua Sha',description:'20 minutes. Facial oil on first, tool gliding at 15 degrees, upward strokes only. Jaw, cheeks, forehead, neck. 10 strokes per section minimum. No rushing. Your face will look visibly different afterwards.',hint:'Apply 3-4 drops of facial oil so the tool glides without dragging. Hold the gua sha nearly flat at 15 degrees to your skin. Always move upward and outward — never back and forth. Start at the neck, then jaw, cheeks, then forehead. 10 slow strokes per section. Finish with gentle pressure under the eyes.',category:'Self-Care',level:2,ctx:'home'},
  {id:'e02',emoji:'💆',title:'Double Cleanse Tonight',description:'First cleanse removes makeup and SPF. Second cleanse actually cleans the skin. Use an oil cleanser first then your regular cleanser. Take two full minutes on each. Massage properly.',hint:'First cleanse: massage an oil cleanser or balm onto dry skin for 60 seconds, emulsify with a little water, rinse. Second cleanse: apply your regular cleanser to damp skin, massage for another 60 seconds focusing on your T-zone and hairline. Rinse thoroughly with lukewarm water. Never hot.',category:'Self-Care',level:1,ctx:'any'},
  {id:'e03',emoji:'💆',title:'Lymphatic Drainage',description:'10 minutes of facial lymphatic drainage massage tonight. Gentle, repetitive downward strokes from face to neck. Reduces puffiness, defines the jaw, clears the skin. Do it every night this week and watch the difference.',hint:'Unlike gua sha, lymphatic drainage uses very light pressure — barely touching the skin. Start at the collarbone with gentle downward strokes. Move up to the neck, then jaw, then under the eyes. The motion is always downward toward the lymph nodes at your collarbone. Never press hard.',category:'Self-Care',level:2,ctx:'home'},
  {id:'e04',emoji:'💆',title:'Hair Mask Protocol',description:'Apply a deep conditioning mask to dry hair before your shower tonight. Leave it for 30 minutes minimum. Rinse thoroughly. Style tomorrow and see the difference one treatment makes.',hint:'Apply to dry hair rather than wet for deeper penetration — start mid-lengths and work to the ends, avoiding the roots. Pile your hair on top of your head and put on a shower cap or wrap in cling film to trap heat. 30 minutes minimum, one hour if you can. Rinse completely with cool water to seal the cuticle.',category:'Self-Care',level:1,ctx:'home'},
  {id:'e05',emoji:'💆',title:'Ice Roller Morning',description:'Keep your ice roller in the freezer and use it every morning before skincare for 5 minutes. Reduces puffiness, closes pores, wakes the skin up completely. Roll outward from the centre of the face.',hint:'Start between the brows, roll outward across the forehead. Then from the nose bridge out across cheeks. Then along the jawline outward. Under the eyes last — very gentle, roll outward only. Never inward toward the nose. 2-3 passes per section is enough. Apply skincare immediately after while pores are tight.',category:'Self-Care',level:2,ctx:'any'},
  {id:'e06',emoji:'💆',title:'Scalp Massage',description:'5 minutes of proper scalp massage tonight — fingertips only, circular motions, working across the whole scalp. Stimulates blood flow, promotes hair growth, and genuinely feels incredible after a long day.',hint:'Use your fingertips not your nails. Work in small circular motions with medium pressure. Start at the base of the skull, work up to the crown, then temples, then hairline. You can do it dry or with a few drops of scalp oil. 5 minutes minimum — set a timer.',category:'Self-Care',level:1,ctx:'home'},
  {id:'e07',emoji:'💆',title:'Body Brush Before Shower',description:'Dry body brush for 5 minutes before your shower tonight. Long upward strokes from feet to heart. Removes dead skin, stimulates circulation, and makes your body lotion absorb three times better afterwards.',hint:'Always brush toward the heart. Start at the soles of your feet, brush upward along the legs in long strokes. Stomach in circular clockwise motions. Arms from hands toward shoulders. Use medium pressure — enough to feel it but not to scratch. Shower immediately after to wash off the dead skin.',category:'Self-Care',level:2,ctx:'home'},
  {id:'e08',emoji:'💆',title:'The Proper Bath',description:'Not a quick wash. A bath. 30 minutes minimum. Magnesium flakes or bath salts, a few drops of oil, candles. Phone in another room. This is a ritual not a task.',category:'Self-Care',level:2,ctx:'home'},
  {id:'e09',emoji:'💆',title:'Vitamin C Serum Morning',description:'Apply your vitamin C serum properly this morning — 3-4 drops patted onto clean dry skin, wait 60 seconds for it to fully absorb before your moisturiser. Done consistently this makes a visible difference within two weeks.',hint:'Apply vitamin C to completely dry skin — wait 60 seconds after cleansing. Pat it on rather than rubbing — back of fingers, gentle pressure across the face. Wait a full minute before the next step. Vitamin C oxidises quickly so keep the bottle away from light and heat.',category:'Self-Care',level:1,ctx:'any'},
  {id:'e10',emoji:'💆',title:'Stretch Protocol Tonight',description:'30 minutes of proper stretching tonight — not casual. Hip flexors held for 90 seconds each side. Hamstrings, chest, shoulders. The areas that tighten from sitting. Hold every stretch for at least 60 seconds.',hint:'The key is duration. Most people hold stretches for 15 seconds which does almost nothing. You need 60-90 seconds minimum for the muscle to actually release. Breathe slowly and lean into the discomfort slightly — not pain, but the edge of the stretch. Hip flexors and chest are the most important if you sit all day.',category:'Self-Care',level:2,ctx:'home'},
  {id:'e11',emoji:'💆',title:'Nail Care Routine',description:'Full nail care tonight — remove old polish, file into proper shape, buff, cuticle oil massaged in, base coat, two colour coats, top coat. Take your time doing each step. Your nails should look salon quality.',hint:'File in one direction only — never sawing back and forth which weakens the nail. Push cuticles back gently after soaking in warm water, never cut them. Apply thin coats rather than thick ones — two thin coats last longer than one thick coat. Seal the tip of the nail with each coat to prevent chipping.',category:'Self-Care',level:2,ctx:'home'},
  {id:'e12',emoji:'💆',title:'Morning Water Ritual',description:'Before anything else this morning — before your phone, before coffee, before anything — drink a full 500ml of water with a squeeze of lemon. Then wait 10 minutes before eating or drinking anything else.',category:'Self-Care',level:1,ctx:'any'},
  {id:'e13',emoji:'💆',title:'Overnight Hair Treatment',description:'Apply a leave-in hair oil or treatment to your lengths and ends tonight before bed. Sleep with your hair in a loose plait. Wake up to softer, shinier hair.',hint:'Apply to damp or dry lengths and ends only — never the roots or it will look greasy. A pea-sized amount of oil is enough. Braid loosely before bed so it does not tangle. In the morning, undo the plait and run your fingers through without brushing immediately — let it settle for a few minutes first.',category:'Self-Care',level:1,ctx:'any'},
  {id:'e14',emoji:'💆',title:'The Real Rest',description:'Two hours this afternoon or evening with no agenda, no productivity, no guilt. Read, nap, lie in the bath, watch something you actually want to watch. Not scrolling. Actual rest. Your body rebuilds during rest.',category:'Self-Care',level:2,ctx:'home'},
  {id:'e15',emoji:'💆',title:'Exfoliate and Glow',description:'Full body exfoliation in the shower — sugar scrub or exfoliating mitt, head to toe. Moisturise immediately after while skin is still slightly damp. Your skin will feel completely different for two days.',hint:'Exfoliate on damp skin using circular motions. Focus extra time on knees, elbows, heels and ankles where skin is roughest. Rinse thoroughly. Pat dry gently rather than rubbing. Apply body lotion immediately while skin is still slightly damp — within 3 minutes of stepping out — to lock in the moisture.',category:'Self-Care',level:1,ctx:'home'},
  {id:'e16',emoji:'💆',title:'Journal Properly',description:'15 minutes of unfiltered writing tonight. Three things that were hard this week and why. Three things you are genuinely proud of. What you want the next 30 days to look like. Do not edit yourself.',category:'Self-Care',level:1,ctx:'home'},
  {id:'e17',emoji:'💆',title:'Cold Water Finish',description:'End every shower this week with 30 seconds of cold water. Just 30 seconds. It closes the hair cuticle, tightens the skin, reduces muscle inflammation and wakes you up completely. It gets easier every time.',category:'Self-Care',level:3,ctx:'any'},
  {id:'e18',emoji:'💆',title:'Dedicated Skincare 15 Minutes',description:'Tonight your skincare is not a 90-second job. Set 15 minutes aside. Double cleanse, exfoliate, toner, serum, eye cream, moisturiser, facial oil to finish. Every product has time to absorb before the next one.',hint:'The rule is 60 seconds between products. Cleanse, rinse, wait. Toner, pat in, wait. Serum, press in with palms, wait a full minute. Eye cream, tap in with ring finger. Moisturiser, press not rub. Facial oil last — press over everything to seal it all in.',category:'Self-Care',level:2,ctx:'home'},
    // ── DEVOTION (15) ────────────────────────────────────────────────────────────
...
function pickSwap(currentCh, excludeUid, seen, seenDates, glowPoints, ovr, custom, naughtyActive, todayDayId) {
  var dayMap = {mon:1,tue:2,wed:3,thu:4,fri:5,sat:6,sun:0};
  var dow = dayMap[todayDayId] !== undefined ? dayMap[todayDayId] : new Date().getDay();
  var validCtx = DAY_CONTEXTS[todayDayId] || ['any'];
  var lvl = getGlowLvl(glowPoints).level;
  var currentIds = currentCh.map(function(c){return c.id;});
  var all = CHALLENGES.concat(custom || []);
  var recentlySeen = Object.keys(seenDates || {}).filter(function(id){
    var d = seenDates[id];
    return d && (new Date().getTime() - new Date(d).getTime()) < 7*24*60*60*1000;
  });
  var pool = all.filter(function(c){
    var o = ovr[c.id] || {};
    if (o.disabled) return false;
    if (currentIds.indexOf(c.id) >= 0) return false;
    if (c.level > lvl) return false;
    if (c.category === 'Naughty') return naughtyActive && lvl >= 2;
    var ctx = c.ctx || 'any';
    return validCtx.indexOf(ctx) >= 0;
  });
  var fresh = pool.filter(function(c){return recentlySeen.indexOf(c.id)<0;});
...
export default function App() {
    ...
    supaLoad().then(function(data){
      if (data) {
        setSt(function(p){ return Object.assign({},p,{
          ...
          naughtyActive:data.naughtyActive||p.naughtyActive, naughtyDate:data.naughtyDate||p.naughtyDate, nextVisit:data.nextVisit||p.nextVisit,
  ...
  useEffect(function(){
    var data = {
      ...
      naughtyActive:st.naughtyActive, naughtyDate:st.naughtyDate, swapUsedDate:st.swapUsedDate, nextVisit:st.nextVisit,
  ...
  }, [st.glowPoints,st.grindPoints,st.ch,st.lgd,st.seen,st.seenDates,st.hist,st.streak,st.lsd,st.graceUsedWeek,st.cats,st.earned,st.msg,st.reaction,st.theme,st.wplan,st.pinSet,st.pin,st.custom,st.ovr,st.weeklyData,st.perfectWeeks,st.pb,st.weekGoal,st.challengeNudgeTime,st.trainingNudgeTime,st.rewards,st.rulesChecked,st.rulesStreak,st.rulesLastDate,st.perfectDays,st.customRules,st.rulesOvr,st.nutDaysLogged,st.naughtyActive,st.naughtyDate,st.swapUsedDate,st.nextVisit,st.herMessages,st.adminSeenDate,st.specialDates,st.milestoneMessages,st.shownMilestones,onboardDone,tChecked,ridesDone,nutChecked,waterMl]);
  ...
  useEffect(function(){
    var interval = setInterval(function(){
      ...
      if (hhmm===st.challengeNudgeTime && localStorage.getItem('lastChallengeNudge')!==today && Notification.permission==='granted') {
        if (doneT<3){new Notification('Glow up time ✨',{body:'Your challenges are waiting. Show Sam what you are made of 💖'});localStorage.setItem('lastChallengeNudge',today);}
      }
  ...
  var cardS={background:'rgba(255,255,255,.042)',border:'1px solid rgba(255,120,190,.18)',borderRadius:20,backdropFilter:'blur(16px)',boxShadow:'0 2px 24px rgba(0,0,0,.18)'};
  var inpS={background:'rgba(255,255,255,.04)',border:'1px solid rgba(255,120,190,.2)',borderRadius:14,padding:'13px 16px',color:'#ffe8f5',fontFamily:'Crimson Text,serif',fontSize:'.95rem',outline:'none',width:'100%',WebkitAppearance:'none',appearance:'none',transition:'border-color .2s'};
  ...
  var gbS={background:grad,border:'none',borderRadius:50,color:'#fff',fontFamily:'Playfair Display,serif',fontSize:'.95rem',fontWeight:700,padding:16,cursor:'pointer',boxShadow:'0 4px 32px '+T.a1+'55,0 0 0 1px '+T.a1+'22',width:'100%',touchAction:'manipulation',letterSpacing:'.02em'};
  ...
  function renderWeekWrap() {
    if (!showWeekWrap) return null;
    return <OVL onClose={function(){setShowWeekWrap(false);}}><div style={Object.assign({},cardS,{padding:'24px',animation:'popIn .3s ease-out',border:'1px solid '+T.a1+'44'})}><div style={{fontFamily:'Playfair Display,serif',fontWeight:900,fontSize:'1.3rem',background:grad,WebkitBackgroundClip:'text',WebkitTextFillColor:'transparent',marginBottom:4}}>Week Wrap 📊</div><div style={{color:'rgba(255,179,217,.4)',fontSize:'0.7rem',marginBottom:14}}>{currentWeekStart}</div>{currentWeekDates.map(function(dateStr){var dayCh=currentWeekData.challenges[dateStr]||[];var doneCh=dayCh.filter(function(c){return c.done;}).length;var d=new Date(dateStr);var dayId2=(['sun','mon','tue','wed','thu','fri','sat'])[d.getDay()];var sess2=SESSIONS[dayId2];var sessRequired=sess2&&(sess2.isGym||sess2.isRide);var sessDone=currentWeekData.trainingSessions[dayId2];var isFuture=new Date(dateStr)>new Date(today);var isPerfect=!isFuture&&doneCh>=3&&(!sessRequired||sessDone);return <div key={dateStr} style={{display:'flex',alignItems:'center',gap:10,padding:'9px 0',borderBottom:'1px solid rgba(255,120,190,.08)'}}><div style={{width:36,textAlign:'center'}}><div style={{fontSize:'0.68rem',fontWeight:'bold',color:isPerfect?T.a1:'rgba(255,179,217,.35)'}}>{DAYS_SHORT[d.getDay()]}</div><div style={{fontSize:'0.58rem',color:'rgba(255,179,217,.25)'}}>{d.getDate()}/{d.getMonth()+1}</div></div><div style={{flex:1}}><div style={{display:'flex',gap:6,flexWrap:'wrap'}}><span style={{fontSize:'0.7rem',color:doneCh>=3?T.a1:'rgba(255,179,217,.3)'}}>✨ {doneCh} challenges</span>{sessRequired&&<span style={{fontSize:'0.7rem',color:sessDone?'#ff3d9a':'rgba(255,179,217,.3)'}}>💪 {sessDone?'done':isFuture?'—':'missed'}</span>}</div></div><div>{isFuture?'':isPerfect?'⭐':'✗'}</div></div>; })}<div style={{marginTop:14,padding:'12px',background:'rgba(255,255,255,.03)',borderRadius:12}}><div style={{display:'flex',justifyContent:'space-between',marginBottom:6}}><span style={{color:'rgba(255,179,217,.5)',fontSize:'0.8rem'}}>Glow</span><span style={{color:T.a1,fontWeight:'bold',fontSize:'0.8rem'}}>{weekGlowBase}{currentWeekData.perfectWeekApplied?' → '+weekGlowTotal:''}</span></div><div style={{display:'flex',justifyContent:'space-between'}}><span style={{color:'rgba(255,179,217,.5)',fontSize:'0.8rem'}}>Grind</span><span style={{color:'#ff3d9a',fontWeight:'bold',fontSize:'0.8rem'}}>{weekGrindBase}{currentWeekData.perfectWeekApplied?' → '+weekGrindTotal:''}</span></div>{currentWeekData.perfectWeekApplied&&<div style={{color:T.a1,fontSize:'0.78rem',fontStyle:'italic',textAlign:'center',marginTop:6}}>🏆 1.5x applied!</div>}</div><button style={Object.assign({},gbS,{marginTop:14})} onClick={function(){setShowWeekWrap(false);}}>Close</button></div></OVL>;
  }

  function renderHomeHeader() {
    var visitCountdown = null;
    if (st.nextVisit) {
      var visitDate = new Date(st.nextVisit);
      visitDate.setHours(0,0,0,0);
      var nowDate = new Date();
      nowDate.setHours(0,0,0,0);
      var diffMs = visitDate - nowDate;
      var diffDays = Math.round(diffMs / (1000*60*60*24));
      if (diffDays >= 0) {
        var visitLabel = diffDays === 0 ? 'Today 💖' : diffDays === 1 ? 'Tomorrow 💖' : diffDays + ' days';
        visitCountdown = <div style={{background:'linear-gradient(135deg,'+T.a1+'22,'+T.a2+'18)',border:'1px solid '+T.a1+'44',borderRadius:20,padding:'14px 20px',marginBottom:14,textAlign:'center',backdropFilter:'blur(12px)',boxShadow:'0 4px 24px '+T.a1+'22'}}><div style={{color:'rgba(255,179,217,.5)',fontSize:'0.6rem',textTransform:'uppercase',letterSpacing:'0.2em',marginBottom:6}}>Next Visit with Sam</div><div style={{fontFamily:'Playfair Display,serif',fontWeight:900,fontSize:'2.2rem',background:'linear-gradient(135deg,'+T.a1+',#ffcce8,'+T.a2+')',WebkitBackgroundClip:'text',WebkitTextFillColor:'transparent',lineHeight:1,marginBottom:4}}>{visitLabel}</div><div style={{color:'rgba(255,179,217,.4)',fontSize:'0.72rem',fontFamily:'Crimson Text,serif',fontStyle:'italic'}}>{new Date(st.nextVisit).toLocaleDateString('en-GB',{weekday:'long',day:'numeric',month:'long'})}</div></div>;
      }
    }
    var streakBadge = st.streak>0?<div style={{display:'inline-flex',alignItems:'center',gap:6,background:'rgba(255,179,71,.08)',border:'1px solid rgba(255,179,71,.2)',borderRadius:50,padding:'7px 14px',marginTop:8}}><span>🔥</span><div style={{color:'#ffb347',fontWeight:'bold',fontFamily:'Playfair Display,serif',fontSize:'0.86rem'}}>{st.streak} day streak</div><span style={{fontSize:'0.8rem'}}>{graceAvailable?'🛡':'💔'}</span></div>:null;
    var glowBar = glowLvi.level<5?<div style={{marginTop:10,padding:'0 20px'}}><div style={{display:'flex',justifyContent:'space-between',marginBottom:3}}><span style={{color:'rgba(255,179,217,.24)',fontSize:'0.58rem'}}>✨ Next: {glowNext.title}</span><span style={{color:'rgba(255,179,217,.24)',fontSize:'0.58rem'}}>{Math.round(glowPct)}%</span></div><div style={{height:4,background:'rgba(255,255,255,.05)',borderRadius:4,overflow:'hidden'}}><div style={{height:'100%',width:glowPct+'%',background:'linear-gradient(90deg,'+T.a1+','+T.a2+')',borderRadius:4,transition:'width .7s'}}/></div></div>:null;
    var grindBar = grindLvi.level<6?<div style={{marginTop:7,padding:'0 20px'}}><div style={{display:'flex',justifyContent:'space-between',marginBottom:3}}><span style={{color:'rgba(255,179,217,.24)',fontSize:'0.58rem'}}>💪 Next: {grindNext.title}</span><span style={{color:'rgba(255,179,217,.24)',fontSize:'0.58rem'}}>{Math.round(grindPct)}%</span></div><div style={{height:4,background:'rgba(255,255,255,.05)',borderRadius:4,overflow:'hidden'}}><div style={{height:'100%',width:grindPct+'%',background:'linear-gradient(90deg,#ff3d9a,#ff85ad)',borderRadius:4,transition:'width .7s'}}/></div></div>:null;
    var weekCard = <div style={{margin:'12px 0 0',padding:'0 4px'}}><div style={Object.assign({},cardS,{padding:'12px 16px',display:'flex',justifyContent:'space-between',alignItems:'center'})}><div style={{textAlign:'left'}}><div style={{color:'rgba(255,179,217,.4)',fontSize:'0.58rem',textTransform:'uppercase',letterSpacing:'0.1em',marginBottom:4}}>This Week</div><div style={{display:'flex',gap:12}}><div><div style={{color:T.a1,fontFamily:'Playfair Display,serif',fontWeight:'bold',fontSize:'0.9rem'}}>{weekGlowTotal} <span style={{fontSize:'0.6rem',color:'rgba(255,179,217,.4)'}}>GP</span></div><div style={{color:'rgba(255,179,217,.3)',fontSize:'0.58rem'}}>Glow</div></div><div><div style={{color:'#ff3d9a',fontFamily:'Playfair Display,serif',fontWeight:'bold',fontSize:'0.9rem'}}>{weekGrindTotal} <span style={{fontSize:'0.6rem',color:'rgba(255,179,217,.4)'}}>GP</span></div><div style={{color:'rgba(255,179,217,.3)',fontSize:'0.58rem'}}>Grind</div></div></div></div><div style={{textAlign:'right'}}>{currentWeekData.perfectWeekApplied&&<div style={{background:'linear-gradient(135deg,'+T.a1+','+T.a2+')',borderRadius:20,padding:'4px 10px',fontSize:'0.68rem',fontWeight:700,color:'#fff',marginBottom:4}}>🏆 1.5x!</div>}<button onClick={function(){setShowWeekWrap(true);}} style={{background:'transparent',border:'1px solid rgba(255,120,190,.24)',borderRadius:20,color:'rgba(255,179,217,.5)',cursor:'pointer',fontSize:'0.68rem',padding:'4px 10px',touchAction:'manipulation'}}>Week Wrap</button></div></div></div>;
    var viewToggle = <div style={{display:'flex',gap:7,justifyContent:'center',marginTop:14}}>{[{id:'her',l:'💅 Her View'},{id:'admin',l:'🔒 Admin'}].map(function(t){return <button key={t.id} style={sbS(st.view===t.id)} onClick={function(){if(t.id==='admin'&&!st.unlocked)tryAdmin();else setSt(function(p){return Object.assign({},p,{view:t.id});});}}>{t.l}</button>;})}</div>;
    return <div><div style={{padding:'16px 15px 0'}}>{visitCountdown}</div><div style={{textAlign:'center',padding:'32px 0 14px'}}><div style={{fontSize:'2.5rem',marginBottom:7,animation:'pulse 3s infinite'}}>💖</div><h1 style={{fontFamily:'Playfair Display,serif',fontWeight:900,fontSize:'clamp(1.8rem,8vw,2.9rem)',background:'linear-gradient(135deg,'+T.a1+',rgba(255,220,240,1),'+T.a2+','+T.a1+')',backgroundSize:'300% 300%',WebkitBackgroundClip:'text',WebkitTextFillColor:'transparent',animation:'shimmer 5s ease infinite',lineHeight:1.1}}>Glow Up</h1><p style={{color:'rgba(255,179,217,.4)',fontStyle:'italic',margin:'6px 0 0',fontSize:'0.88rem',fontFamily:'Crimson Text,serif'}}>your daily challenges ✨</p><div style={{display:'flex',gap:8,justifyContent:'center',marginTop:13,flexWrap:'wrap'}}><div style={{display:'inline-flex',alignItems:'center',gap:9,background:T.a1+'12',border:'1px solid '+T.a1+'22',borderRadius:50,padding:'9px 18px'}}><span style={{fontSize:'1.3rem'}}>{glowLvi.emoji}</span><div style={{textAlign:'left'}}><div style={{color:glowLvi.color,fontWeight:'bold',fontFamily:'Playfair Display,serif',fontSize:'0.86rem'}}>{glowLvi.title}</div><div style={{color:'rgba(255,179,217,.3)',fontSize:'0.58rem'}}>✨ {st.glowPoints} Glow Pts</div></div></div><div style={{display:'inline-flex',alignItems:'center',gap:9,background:'rgba(255,45,120,.08)',border:'1px solid rgba(255,45,120,.2)',borderRadius:50,padding:'9px 18px'}}><span style={{fontSize:'1.3rem'}}>{grindLvi.emoji}</span><div style={{textAlign:'left'}}><div style={{color:grindLvi.color,fontWeight:'bold',fontFamily:'Playfair Display,serif',fontSize:'0.86rem'}}>{grindLvi.title}</div><div style={{color:'rgba(255,179,217,.3)',fontSize:'0.58rem'}}>💪 {st.grindPoints} Grind Pts</div></div></div></div>{streakBadge}{glowBar}{grindBar}{weekCard}{st.adminSeenDate===today&&<div style={{marginTop:8,fontSize:'0.72rem',color:T.a1+'88',fontFamily:'Crimson Text,serif',fontStyle:'italic'}}>Sam has seen today 💖</div>}{viewToggle}</div></div>;
  }
  function renderChallengeCard(c, i) {
    var cc = CAT_COLOR[c.category]||'#ff6eb4';
    return <div key={c.uid} style={Object.assign({},cardS,{padding:'18px',animation:'popIn .4s '+(i*.12)+'s ease-out both',border:c.done?'1px solid '+T.a1+'55':'1px solid rgba(255,120,190,.13)',background:c.done?T.a1+'09':'rgba(255,255,255,.032)',boxShadow:c.done?'0 0 20px '+T.a1+'18,inset 0 0 20px '+T.a1+'05':'none',position:'relative',overflow:'hidden'})}>{st.celebId===c.uid&&<div style={{position:'absolute',inset:0,display:'flex',alignItems:'center',justifyContent:'center',fontSize:'2rem',pointerEvents:'none',zIndex:5}}>✨💖✨</div>}<div style={{position:'absolute',top:0,left:0,width:'3px',height:'100%',background:'linear-gradient(to bottom,'+cc+','+cc+'44)',borderRadius:'3px 0 0 3px'}}/><div style={{display:'flex',alignItems:'flex-start',gap:12,paddingLeft:7}}><button style={cbS(c.done)} onClick={function(){complete(c.uid);}}>{c.done?'✓':''}</button><div style={{flex:1,minWidth:0}}><div style={{display:'flex',alignItems:'center',gap:6,flexWrap:'wrap',marginBottom:5}}><span style={{fontSize:'1.2rem'}}>{c.emoji}</span><span style={{fontFamily:'Playfair Display,serif',fontWeight:'bold',fontSize:'0.96rem',color:c.done?'rgba(255,179,217,.32)':'#ffe8f5',textDecoration:c.done?'line-through':'none',lineHeight:1.3}}>{c.title}</span><span style={{fontSize:'0.58rem',padding:'2px 9px',borderRadius:50,background:cc+'14',color:cc,border:'1px solid '+cc+'30',textTransform:'uppercase',whiteSpace:'nowrap'}}>{CAT_ICON[c.category]} {c.category}</span></div><p style={{margin:0,color:c.done?'rgba(255,179,217,.22)':'rgba(255,179,217,.55)',fontSize:'0.87rem',fontFamily:'Crimson Text,serif',lineHeight:1.65}}>{c.description}</p>{c.hint&&!c.done&&<div style={{marginTop:8,padding:'8px 11px',background:'rgba(255,255,255,.03)',borderRadius:10,border:'1px solid rgba(255,120,190,.1)'}}><div style={{color:'rgba(255,179,217,.4)',fontSize:'0.6rem',textTransform:'uppercase',letterSpacing:'0.1em',marginBottom:4}}>💡 How to</div><p style={{color:'rgba(255,179,217,.45)',fontSize:'0.78rem',fontFamily:'Crimson Text,serif',lineHeight:1.5,margin:0}}>{c.hint}</p></div>}{c.done&&<div style={{fontSize:'0.7rem',color:T.a1,marginTop:5}}>+{GLOW_PER_CHALLENGE} ✨ Glow Points</div>}{c.done&&<div style={{display:'flex',gap:8,marginTop:8,flexWrap:'wrap'}}><button onClick={function(){setSt(function(p){return Object.assign({},p,{noteId:c.uid,noted:c.note||''});});}} style={{background:'transparent',border:'1px solid rgba(255,120,190,.22)',borderRadius:20,color:'rgba(255,179,217,.5)',cursor:'pointer',fontSize:'0.7rem',padding:'4px 12px',touchAction:'manipulation'}}>{c.note?'📝 Edit':'📝 Note'}</button><button onClick={function(){setSt(function(p){return Object.assign({},p,{diffId:c.uid});});}} style={{background:'transparent',border:'1px solid rgba(255,120,190,.22)',borderRadius:20,color:'rgba(255,179,217,.5)',cursor:'pointer',fontSize:'0.7rem',padding:'4px 12px',touchAction:'manipulation'}}>{c.diff?(c.diff==='Easy'?'😊 ':c.diff==='Just Right'?'💪 ':'🔥 ')+c.diff:'⭐ Rate'}</button></div>}{!c.done&&canSwap&&<button onClick={function(){swapChallenge(c.uid);}} style={{marginTop:8,background:'transparent',border:'1px solid rgba(255,120,190,.18)',borderRadius:20,color:'rgba(255,179,217,.3)',cursor:'pointer',fontSize:'0.68rem',padding:'3px 10px',touchAction:'manipulation'}}>🔄 Swap</button>}{c.note&&<p style={{margin:'7px 0 0',color:'rgba(255,179,217,.4)',fontSize:'0.78rem',fontFamily:'Crimson Text,serif',fontStyle:'italic'}}>{c.note}</p>}</div></div></div>;
  }

  function renderHerToday() {
    var reactionCard = st.reaction?<div style={{background:'linear-gradient(135deg,'+T.a1+'20,'+T.a2+'15)',border:'1px solid '+T.a1+'55',borderRadius:20,padding:'20px',marginBottom:14,boxShadow:'0 8px 32px '+T.a1+'25,inset 0 0 40px '+T.a1+'08',backdropFilter:'blur(16px)'}}><div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:10}}><div style={{display:'flex',alignItems:'center',gap:8}}><span style={{fontSize:'1.6rem',animation:'pulse 2s ease-in-out infinite'}}>💌</span><div style={{color:T.a1,fontSize:'0.62rem',textTransform:'uppercase',letterSpacing:'0.18em',fontWeight:'bold'}}>Sam sent you something</div></div><button onClick={function(){setSt(function(p){return Object.assign({},p,{reaction:''});});}} style={{background:'rgba(255,255,255,.06)',border:'none',borderRadius:50,color:'rgba(255,179,217,.5)',cursor:'pointer',fontSize:'0.7rem',padding:'4px 10px',touchAction:'manipulation'}}>dismiss</button></div><div style={{color:'#ffe8f5',fontFamily:'Playfair Display,serif',fontSize:'1.05rem',lineHeight:1.7,fontStyle:'italic'}}>{st.reaction}</div></div>:null;
    var msgCard2 = st.msg?<div style={{background:'linear-gradient(135deg,'+T.a1+'15,'+T.a2+'0a)',border:'1px solid '+T.a1+'44',borderRadius:20,padding:'18px 20px',marginBottom:14,backdropFilter:'blur(12px)',boxShadow:'0 4px 20px '+T.a1+'18'}}><div style={{display:'flex',alignItems:'center',gap:10,marginBottom:8}}><span style={{fontSize:'1.4rem'}}>💌</span><div style={{color:T.a1,fontSize:'0.62rem',textTransform:'uppercase',letterSpacing:'0.15em',fontWeight:'bold'}}>From Sam</div></div><div style={{color:'#ffe8f5',fontFamily:'Crimson Text,serif',fontSize:'1rem',fontStyle:'italic',lineHeight:1.65}}>{st.msg}</div></div>:null;
    var progressBar = st.ch.length>0?<div style={Object.assign({},cardS,{padding:'13px 16px',marginBottom:13})}><div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:7}}><span style={{color:'rgba(255,179,217,.44)',fontSize:'0.64rem',textTransform:'uppercase',letterSpacing:'0.12em'}}>Today — all 3 needed</span><span style={{color:doneT>=3?T.a1:'rgba(255,179,217,.5)',fontWeight:'bold',fontFamily:'Playfair Display,serif'}}>{doneT} / {st.ch.length}</span></div><div style={{height:7,background:'rgba(255,255,255,.05)',borderRadius:7,overflow:'hidden'}}><div style={{height:'100%',width:(st.ch.length?(doneT/st.ch.length)*100:0)+'%',background:'linear-gradient(90deg,'+T.a1+','+T.a2+')',borderRadius:7,transition:'width .6s'}}/></div></div>:null;
    var herMsgBox = <div style={Object.assign({},cardS,{padding:'14px',marginTop:14})}><div style={{color:'rgba(255,179,217,.4)',fontSize:'0.6rem',textTransform:'uppercase',letterSpacing:'0.1em',marginBottom:8}}>💬 Message Sam</div>{(st.herMessages||[]).filter(function(m){return m.date===today;}).map(function(m,i){return <div key={i} style={{padding:'8px 11px',background:T.a1+'0a',borderRadius:10,marginBottom:6,fontSize:'0.82rem',color:'rgba(255,179,217,.65)',fontFamily:'Crimson Text,serif',fontStyle:'italic'}}>{m.text} {m.read&&<span style={{fontSize:'0.6rem',color:T.a1}}>✓ seen</span>}</div>; })}<div style={{display:'flex',gap:8,marginTop:6}}><input placeholder='Say something to Sam...' value={st.herMsgDraft} onChange={function(e){setSt(function(p){return Object.assign({},p,{herMsgDraft:e.target.value});});}} style={Object.assign({},inpS,{flex:1,padding:'10px 12px'})} onKeyDown={function(e){if(e.key==='Enter'&&st.herMsgDraft.trim()){var msg={id:Date.now(),text:st.herMsgDraft.trim(),date:today,read:false};setSt(function(p){return Object.assign({},p,{herMessages:(p.herMessages||[]).concat([msg]),herMsgDraft:''});});}}}/><button onClick={function(){if(!st.herMsgDraft.trim())return;var msg={id:Date.now(),text:st.herMsgDraft.trim(),date:today,read:false};setSt(function(p){return Object.assign({},p,{herMessages:(p.herMessages||[]).concat([msg]),herMsgDraft:''});});}} style={{background:grad,border:'none',borderRadius:12,color:'#fff',padding:'10px 14px',cursor:'pointer',fontFamily:'Playfair Display,serif',fontWeight:700,fontSize:'0.82rem',touchAction:'manipulation',flexShrink:0}}>Send</button></div></div>;
    return <div>{reactionCard}{msgCard2}{progressBar}<div style={{display:"flex",flexDirection:"column",gap:12}}>{st.ch.map(renderChallengeCard)}</div>{herMsgBox}{!canSwap&&<div style={{textAlign:"center",color:"rgba(255,179,217,.25)",fontSize:"0.66rem",fontStyle:"italic",marginTop:8}}>Swap used for today</div>}</div>;
  }

  function renderHerStats() {
    var topCards = <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10,marginBottom:14}}><div style={Object.assign({},cardS,{padding:'16px 12px',textAlign:'center',border:'1px solid '+T.a1+'33'})}><div style={{fontSize:'1.8rem',marginBottom:5}}>{glowLvi.emoji}</div><div style={{color:glowLvi.color,fontFamily:'Playfair Display,serif',fontWeight:'bold',fontSize:'1rem'}}>{glowLvi.title}</div><div style={{color:T.a1,fontFamily:'Playfair Display,serif',fontWeight:'bold',fontSize:'1.4rem',marginTop:2}}>{st.glowPoints}</div><div style={{color:'rgba(255,179,217,.32)',fontSize:'0.6rem',textTransform:'uppercase',letterSpacing:'0.07em'}}>Glow Points</div></div><div style={Object.assign({},cardS,{padding:'16px 12px',textAlign:'center',border:'1px solid rgba(255,45,120,.22)'})}><div style={{fontSize:'1.8rem',marginBottom:5}}>{grindLvi.emoji}</div><div style={{color:grindLvi.color,fontFamily:'Playfair Display,serif',fontWeight:'bold',fontSize:'1rem'}}>{grindLvi.title}</div><div style={{color:'#ff3d9a',fontFamily:'Playfair Display,serif',fontWeight:'bold',fontSize:'1.4rem',marginTop:2}}>{st.grindPoints}</div><div style={{color:'rgba(255,179,217,.32)',fontSize:'0.6rem',textTransform:'uppercase',letterSpacing:'0.07em'}}>Grind Points</div></div></div>;
    var statCards = <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10,marginBottom:14}}>{[{e:'🔥',v:st.streak,l:'Day Streak'},{e:'🏆',v:st.perfectWeeks||0,l:'Perfect Weeks'},{e:'✨',v:st.pb.bestGlowWeek||0,l:'Best Glow Week'},{e:'💪',v:st.pb.bestGrindWeek||0,l:'Best Grind Week'}].map(function(s,i){return <div key={i} style={Object.assign({},cardS,{padding:'14px 12px',textAlign:'center'})}><div style={{fontSize:'1.5rem',marginBottom:5}}>{s.e}</div><div style={{color:'#ffe8f5',fontFamily:'Playfair Display,serif',fontWeight:'bold',fontSize:'1rem'}}>{s.v}</div><div style={{color:'rgba(255,179,217,.32)',fontSize:'0.6rem',marginTop:3,textTransform:'uppercase',letterSpacing:'0.07em'}}>{s.l}</div></div>; })}</div>;
    var catChart = <div style={Object.assign({},cardS,{padding:'16px',marginBottom:13})}><div style={{color:'rgba(255,179,217,.42)',fontSize:'0.64rem',textTransform:'uppercase',letterSpacing:'0.12em',marginBottom:12}}>By Category</div>{CATS.map(function(cat){var n=st.cats[cat]||0;var mx=Math.max.apply(null,CATS.map(function(c){return st.cats[c]||0;}).concat([1]));var cc=CAT_COLOR[cat];return <div key={cat} style={{marginBottom:9}}><div style={{display:'flex',justifyContent:'space-between',marginBottom:3}}><span style={{color:'rgba(255,179,217,.52)',fontSize:'0.76rem'}}>{CAT_ICON[cat]} {cat}</span><span style={{color:cc,fontSize:'0.76rem',fontWeight:'bold'}}>{n}</span></div><div style={{height:4,background:'rgba(255,255,255,.05)',borderRadius:4,overflow:'hidden'}}><div style={{height:'100%',width:((n/mx)*100)+'%',background:'linear-gradient(90deg,'+cc+','+cc+'88)',borderRadius:4}}/></div></div>; })}</div>;
    var achGrid = <div style={Object.assign({},cardS,{padding:'16px'})}><div style={{color:'rgba(255,179,217,.42)',fontSize:'0.64rem',textTransform:'uppercase',letterSpacing:'0.12em',marginBottom:12}}>Achievements ({st.earned.length}/{ACHIEVEMENTS.length})</div><div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8}}>{ACHIEVEMENTS.map(function(a){var e=st.earned.indexOf(a.id)>=0;return <div key={a.id} style={{background:e?T.a1+'12':'rgba(255,255,255,.02)',border:'1px solid '+(e?T.a1+'44':'rgba(255,120,190,.1)'),borderRadius:12,padding:'11px 9px',textAlign:'center',opacity:e?1:0.36}}><div style={{fontSize:'1.3rem',marginBottom:3}}>{a.emoji}</div><div style={{color:e?'#ffe8f5':'rgba(255,179,217,.46)',fontFamily:'Playfair Display,serif',fontSize:'0.74rem',fontWeight:'bold',marginBottom:2}}>{a.title}</div><div style={{color:'rgba(255,179,217,.3)',fontSize:'0.58rem',lineHeight:1.3}}>{a.desc}</div></div>; })}</div></div>;
    return <div style={{animation:"up .35s ease-out"}}>{topCards}{statCards}{catChart}{achGrid}</div>;
  }

  function renderHerHistory() {
    return <div style={{animation:'up .35s ease-out'}}><div style={{color:'rgba(255,179,217,.32)',fontSize:'0.64rem',textTransform:'uppercase',letterSpacing:'0.14em',marginBottom:12,textAlign:'center'}}>Past Challenges</div>{hdates.length===0?<div style={{textAlign:'center',padding:'50px 20px',color:'rgba(255,179,217,.26)',fontStyle:'italic',fontFamily:'Crimson Text,serif'}}>Complete some challenges and they will appear here 🌸</div>:hdates.map(function(date){var dc=st.hist[date]||[];var dd=dc.filter(function(c){return c.done;}).length;var pf=dd>=3;return <div key={date} style={Object.assign({},cardS,{padding:'13px 16px',marginBottom:10,border:pf?'1px solid '+T.a1+'30':'1px solid rgba(255,120,190,.11)'})}><div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:7}}><span style={{color:'rgba(255,179,217,.48)',fontSize:'0.78rem'}}>{date}</span><span style={{fontSize:'0.72rem',color:pf?T.a1:'rgba(255,179,217,.3)'}}>{dd}/{dc.length}{pf?' ✨':''}</span></div>{dc.map(function(c,i){return <div key={i}><div style={{display:'flex',alignItems:'center',gap:8,marginTop:5,opacity:c.done?1:0.32}}><span style={{fontSize:'0.78rem'}}>{c.done?'✅':'⭕'}</span><span style={{color:'rgba(255,179,217,.55)',fontSize:'0.8rem',flex:1}}>{c.title}</span>{c.diff&&<span style={{fontSize:'0.7rem'}}>{c.diff==='Easy'?'😊':c.diff==='Just Right'?'💪':'🔥'}</span>}</div>{c.note&&<div style={{color:'rgba(255,179,217,.35)',fontSize:'0.72rem',fontStyle:'italic',marginLeft:22,marginTop:2}}>{c.note}</div>}</div>; })}</div>; })}</div>;
  }

  function renderAdminToday() {
    var statsGrid = <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8,marginBottom:13}}>{[{l:'Glow Pts',v:st.glowPoints,e:'✨'},{l:'Grind Pts',v:st.grindPoints,e:'💪'},{l:'Streak',v:st.streak+'d 🔥',e:'📅'},{l:'Perfect Wks',v:st.perfectWeeks||0,e:'🏆'}].map(function(s,i){return <div key={i} style={Object.assign({},cardS,{padding:'12px 7px',textAlign:'center'})}><div style={{fontSize:'1.2rem',marginBottom:3}}>{s.e}</div><div style={{color:'#ffb3d9',fontFamily:'Playfair Display,serif',fontWeight:'bold',fontSize:'0.8rem'}}>{s.v}</div><div style={{color:'rgba(255,179,217,.3)',fontSize:'0.58rem',marginTop:2,textTransform:'uppercase',letterSpacing:'0.07em'}}>{s.l}</div></div>; })}</div>;
    var msgs = (st.herMessages||[]).length>0?<div style={Object.assign({},cardS,{padding:'14px',marginBottom:11})}><div style={lblS}>Messages from Rebecca {unreadHerMsgs>0&&<span style={{background:T.a1,color:'#fff',borderRadius:20,padding:'1px 7px',fontSize:'0.6rem',marginLeft:6}}>{unreadHerMsgs}</span>}</div>{(st.herMessages||[]).slice(-5).reverse().map(function(m){return <div key={m.id} style={{padding:'8px 11px',background:m.read?'rgba(255,255,255,.02)':T.a1+'0a',borderRadius:10,marginBottom:6,border:'1px solid '+(m.read?'rgba(255,120,190,.1)':T.a1+'22')}}><div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',gap:8}}><div style={{color:'#ffe8f5',fontSize:'0.84rem',fontFamily:'Crimson Text,serif',fontStyle:'italic',flex:1}}>{m.text}</div><button onClick={function(){setSt(function(p){return Object.assign({},p,{herMessages:p.herMessages.map(function(x){return x.id===m.id?Object.assign({},x,{read:true}):x;})});});}} style={{background:'transparent',border:'none',color:m.read?'rgba(255,179,217,.3)':T.a1,cursor:'pointer',fontSize:'0.7rem',padding:'2px 6px',touchAction:'manipulation',flexShrink:0}}>{m.read?'✓ seen':'Mark seen'}</button></div><div style={{color:'rgba(255,179,217,.3)',fontSize:'0.62rem',marginTop:3}}>{m.date}</div></div>; })}</div>:null;
    var pinCard = <div style={Object.assign({},cardS,{padding:'14px',marginBottom:11})}><div style={lblS}>Quick Pin</div><select value={st.qpin||''} onChange={function(e){setSt(function(p){return Object.assign({},p,{qpin:e.target.value||null});});}} style={Object.assign({},inpS,{marginBottom:9})}><option value=''>Pick a challenge...</option>{allCh.map(function(c){return <option key={c.id} value={c.id}>{c.emoji} {c.title}</option>;})}</select><button style={gbS} onClick={function(){if(!st.qpin)return;var ch=allCh.find(function(c){return c.id===st.qpin;});if(!ch)return;setSt(function(p){var ex=p.ch.filter(function(c){return c.id!==ch.id;});var nc=[Object.assign({},ch,{uid:today+'-pin',done:false,note:'',diff:null})].concat(ex).slice(0,3);return Object.assign({},p,{ch:nc,lgd:today,qpin:null});});}}>Pin Challenge 📌</button></div>;
    var msgCard = <div style={Object.assign({},cardS,{padding:'14px',marginBottom:11})}><div style={lblS}>Message for her</div><textarea rows={2} value={st.msgd} onChange={function(e){setSt(function(p){return Object.assign({},p,{msgd:e.target.value});});}} style={Object.assign({},inpS,{resize:'none',lineHeight:1.5,marginBottom:9})}/><button style={gbS} onClick={function(){setSt(function(p){return Object.assign({},p,{msg:p.msgd});});}}>Save 💕</button></div>;
    var reactCard = <div style={Object.assign({},cardS,{padding:'14px',marginBottom:11})}><div style={lblS}>Send a reaction</div><textarea rows={2} value={st.reactd} onChange={function(e){setSt(function(p){return Object.assign({},p,{reactd:e.target.value});});}} style={Object.assign({},inpS,{resize:'none',lineHeight:1.5,marginBottom:9})}/><button style={gbS} onClick={function(){setSt(function(p){return Object.assign({},p,{reaction:p.reactd,reactd:''});});}}>Send 💖</button></div>;
    var todayChallenges = st.ch.length>0?<div style={{marginBottom:13}}><div style={lblS}>Todays Challenges</div>{st.ch.map(function(c){var cc=CAT_COLOR[c.category]||'#ff6eb4';return <div key={c.uid} style={Object.assign({},cardS,{padding:'10px 13px',marginBottom:7,display:'flex',alignItems:'center',gap:9})}><span>{c.emoji}</span><div style={{flex:1,minWidth:0}}><div style={{color:'#ffe8f5',fontFamily:'Playfair Display,serif',fontSize:'0.82rem',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{c.title}</div><div style={{color:cc,fontSize:'0.6rem',marginTop:2}}>{c.category} · {c.done?'done':'pending'}</div></div></div>; })}</div>:null;
    return <div>{statsGrid}{msgs}{pinCard}{msgCard}{reactCard}<button style={Object.assign({},gbS,{marginBottom:6})} onClick={function(){gen();}}>🔄 Regenerate</button><p style={{color:'rgba(255,179,217,.24)',fontSize:'0.7rem',textAlign:'center',marginBottom:13,fontStyle:'italic'}}>Context: {(DAY_CONTEXTS[todayDayId]||[]).join(', ')}</p>{todayChallenges}<button onClick={function(){setSt(function(p){return Object.assign({},p,{unlocked:false,view:'her'});});}} style={Object.assign({},sbS(false),{width:'100%',padding:'11px'})}>🔒 Lock</button></div>;
  }

  function renderAdminChallenges() {
    return <div><input placeholder='Search...' value={st.fsearch} onChange={function(e){setSt(function(p){return Object.assign({},p,{fsearch:e.target.value});});}} style={Object.assign({},inpS,{marginBottom:9})}/><div style={{display:'flex',gap:5,overflowX:'auto',padding:'2px 0',marginBottom:11}}>{['All'].concat(CATS).map(function(cat){return <button key={cat} style={Object.assign({},sbS(st.fcat===cat),{fontSize:'0.68rem',padding:'6px 11px',whiteSpace:'nowrap'})} onClick={function(){setSt(function(p){return Object.assign({},p,{fcat:cat});});}}>{cat==='All'?'All':CAT_ICON[cat]+' '+cat}</button>; })}</div><div style={{display:'flex',flexDirection:'column',gap:7}}>{filt.map(function(c){var ov=st.ovr[c.id]||{};var dis=!!ov.disabled;var sched=ov.rt&&ov.rt!=='none';var cc=CAT_COLOR[c.category]||'#ff6eb4';return <div key={c.id} style={Object.assign({},cardS,{padding:'10px 12px',opacity:dis?0.42:1,border:sched?'1px solid '+T.a1+'35':'1px solid rgba(255,120,190,.12)'})}><div style={{display:'flex',alignItems:'center',gap:8}}><span style={{fontSize:'0.95rem'}}>{c.emoji}</span><div style={{flex:1,minWidth:0}}><div style={{color:'#ffe8f5',fontFamily:'Playfair Display,serif',fontSize:'0.8rem',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap',textDecoration:dis?'line-through':'none'}}>{c.title}</div><div style={{display:'flex',gap:5,marginTop:2,flexWrap:'wrap'}}><span style={{color:cc,fontSize:'0.58rem'}}>{CAT_ICON[c.category]} {c.category}</span><span style={{color:'rgba(255,179,217,.3)',fontSize:'0.56rem'}}>{c.ctx||'any'}</span>{c.custom&&<span style={{color:'rgba(255,179,71,.5)',fontSize:'0.56rem'}}>custom</span>}</div></div><button onClick={function(){setSt(function(p){return Object.assign({},p,{schedId:c.id});});}} style={{background:'transparent',border:'1px solid rgba(255,120,190,.18)',borderRadius:8,color:sched?T.a1:'rgba(255,179,217,.34)',cursor:'pointer',fontSize:'0.66rem',padding:'4px 8px',touchAction:'manipulation'}}>📅</button><button onClick={function(){setOv(c.id,{disabled:!dis});}} style={{width:44,height:24,borderRadius:12,border:'none',cursor:'pointer',background:dis?'rgba(255,120,190,.18)':'linear-gradient(135deg,'+T.a1+','+T.a2+')',position:'relative',touchAction:'manipulation',flexShrink:0}}><div style={{position:'absolute',top:3,left:dis?3:23,width:18,height:18,borderRadius:'50%',background:'#fff',transition:'left .2s'}}/></button></div>{c.custom&&<button onClick={function(){setSt(function(p){return Object.assign({},p,{custom:p.custom.filter(function(x){return x.id!==c.id;})});});}} style={{background:'transparent',border:'none',color:'rgba(255,100,100,.34)',cursor:'pointer',fontSize:'0.66rem',marginTop:6,padding:0,touchAction:'manipulation'}}>Delete</button>}</div>; })}</div></div>;
  }

  function renderAdminPlanner() {
    return <div><p style={{color:'rgba(255,179,217,.4)',fontSize:'0.78rem',fontFamily:'Crimson Text,serif',marginBottom:13,lineHeight:1.5}}>Set specific challenges for any day this week.</p>{getWeekDatesUI().map(function(date,i){var ds=date.toDateString();var isTd=ds===today;var isPast=date<new Date()&&!isTd;var planned=st.wplan[ds]||[];var exp=st.wpexp===ds;return <div key={i} style={Object.assign({},cardS,{marginBottom:8,border:isTd?'1px solid '+T.a1+'44':'1px solid rgba(255,120,190,.13)',opacity:isPast?0.5:1})}><button onClick={function(){setSt(function(p){return Object.assign({},p,{wpexp:p.wpexp===ds?null:ds});});}} style={{width:'100%',background:'transparent',border:'none',padding:'12px 14px',cursor:'pointer',display:'flex',alignItems:'center',gap:10,touchAction:'manipulation'}}><div style={{flex:1,textAlign:'left'}}><div style={{color:isTd?T.a1:'#ffe8f5',fontFamily:'Playfair Display,serif',fontSize:'0.88rem',fontWeight:isTd?'bold':'normal'}}>{DAYS_SHORT[date.getDay()]} {date.getDate()}/{date.getMonth()+1}{isTd?' (Today)':''}</div><div style={{color:'rgba(255,179,217,.38)',fontSize:'0.64rem',marginTop:2}}>{planned.length>0?planned.length+' planned':'Free choice'}</div></div><span style={{color:'rgba(255,179,217,.3)',fontSize:'0.8rem'}}>{exp?'▲':'▼'}</span></button>{exp&&<div style={{padding:'0 14px 14px'}}>{planned.map(function(id){var ch=allCh.find(function(c){return c.id===id;});return ch?<div key={id} style={{display:'flex',alignItems:'center',gap:8,marginBottom:6,padding:'8px 10px',background:'rgba(255,255,255,.03)',borderRadius:10,border:'1px solid '+T.a1+'22'}}><span style={{fontSize:'0.9rem'}}>{ch.emoji}</span><span style={{color:'rgba(255,179,217,.65)',fontSize:'0.78rem',flex:1}}>{ch.title}</span><button onClick={function(){setSt(function(p){var np=Object.assign({},p.wplan);np[ds]=(np[ds]||[]).filter(function(x){return x!==id;});return Object.assign({},p,{wplan:np});});}} style={{background:'transparent',border:'none',color:'rgba(255,100,100,.38)',cursor:'pointer',fontSize:'1rem',padding:'2px',touchAction:'manipulation'}}>x</button></div>:null; })}{planned.length<3&&<select onChange={function(e){if(e.target.value&&planned.indexOf(e.target.value)<0){setSt(function(p){var np=Object.assign({},p.wplan);np[ds]=(np[ds]||[]).concat([e.target.value]);return Object.assign({},p,{wplan:np});});}e.target.value='';}} defaultValue='' style={inpS}><option value=''>Add a challenge...</option>{allCh.filter(function(c){return planned.indexOf(c.id)<0;}).map(function(c){return <option key={c.id} value={c.id}>{c.emoji} {c.title}</option>;})}</select>}{planned.length>0&&<button onClick={function(){setSt(function(p){var np=Object.assign({},p.wplan);np[ds]=[];return Object.assign({},p,{wplan:np});});}} style={{background:'transparent',border:'none',color:'rgba(255,100,100,.38)',cursor:'pointer',fontSize:'0.7rem',marginTop:7,padding:0}}>Clear day</button>}</div>}</div>; })}</div>;
  }

  function renderAdminAddNew() {
    return <div><div style={{marginBottom:10}}><div style={lblS}>Title</div><input placeholder='Short punchy title...' value={st.form.title} onChange={function(e){setSt(function(p){return Object.assign({},p,{form:Object.assign({},p.form,{title:e.target.value}),ferr:''});});}} style={inpS}/></div><div style={{marginBottom:10}}><div style={lblS}>Description</div><textarea rows={3} value={st.form.desc} onChange={function(e){setSt(function(p){return Object.assign({},p,{form:Object.assign({},p.form,{desc:e.target.value}),ferr:''});});}} style={Object.assign({},inpS,{resize:'none',lineHeight:1.5})}/></div><div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:9,marginBottom:10}}><div><div style={lblS}>Category</div><select value={st.form.category} onChange={function(e){setSt(function(p){return Object.assign({},p,{form:Object.assign({},p.form,{category:e.target.value})});});}} style={inpS}>{CATS.map(function(c){return <option key={c} value={c}>{CAT_ICON[c]} {c}</option>;})}</select></div><div><div style={lblS}>Emoji</div><input value={st.form.emoji} onChange={function(e){setSt(function(p){return Object.assign({},p,{form:Object.assign({},p.form,{emoji:e.target.value})});});}} style={Object.assign({},inpS,{textAlign:'center',fontSize:'1.3rem'})} maxLength={2}/></div></div><div style={{marginBottom:13}}><div style={lblS}>Level</div><select value={st.form.level} onChange={function(e){setSt(function(p){return Object.assign({},p,{form:Object.assign({},p.form,{level:Number(e.target.value)})});});}} style={inpS}>{GLOW_LEVELS.map(function(l){return <option key={l.level} value={l.level}>{l.emoji} Level {l.level} — {l.title}</option>;})}</select></div>{st.ferr&&<p style={{color:'#ff3d9a',fontSize:'0.82rem',marginBottom:10}}>{st.ferr}</p>}<button style={gbS} onClick={addOne}>Add Challenge 💕</button></div>;
  }

  function renderAdminBulk() {
    return <div><div style={Object.assign({},cardS,{padding:'13px',marginBottom:12})}><div style={{color:T.a1,fontFamily:'Playfair Display,serif',fontSize:'0.88rem',fontWeight:'bold',marginBottom:6}}>How it works</div><div style={{color:'rgba(255,179,217,.52)',fontSize:'0.78rem',fontFamily:'Crimson Text,serif',lineHeight:1.6}}>One challenge per line. Optional description on the next line.</div></div><div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:9,marginBottom:10}}><div><div style={lblS}>Category</div><select value={st.bcat} onChange={function(e){setSt(function(p){return Object.assign({},p,{bcat:e.target.value});});}} style={inpS}>{CATS.map(function(c){return <option key={c} value={c}>{CAT_ICON[c]} {c}</option>;})}</select></div><div><div style={lblS}>Emoji</div><input value={st.bemoji} onChange={function(e){setSt(function(p){return Object.assign({},p,{bemoji:e.target.value});});}} style={Object.assign({},inpS,{textAlign:'center',fontSize:'1.3rem'})} maxLength={2}/></div></div><div style={{marginBottom:10}}><div style={lblS}>Level</div><select value={st.blvl} onChange={function(e){setSt(function(p){return Object.assign({},p,{blvl:Number(e.target.value)});});}} style={inpS}>{GLOW_LEVELS.map(function(l){return <option key={l.level} value={l.level}>{l.emoji} Level {l.level} — {l.title}</option>;})}</select></div><div style={{marginBottom:10}}><div style={lblS}>Paste challenges</div><textarea rows={8} value={st.bulk} onChange={function(e){setSt(function(p){return Object.assign({},p,{bulk:e.target.value,bres:''});});}} style={Object.assign({},inpS,{resize:'none',lineHeight:1.5,fontFamily:'monospace',fontSize:'0.8rem'})}/></div>{st.bres&&<div style={{color:st.bres.indexOf('Imported')>=0?'#90caf9':'#ff6eb4',fontSize:'0.8rem',marginBottom:10,padding:'9px 13px',background:'rgba(255,255,255,.04)',borderRadius:10}}>{st.bres}</div>}<button style={gbS} onClick={bulkImport}>Import</button></div>;
  }

  function renderAdminWeek() {
    return <div><div style={{fontFamily:'Playfair Display,serif',fontWeight:'bold',fontSize:'1.1rem',color:'#ffe8f5',marginBottom:4}}>Weekly Summary</div><div style={{color:'rgba(255,179,217,.4)',fontSize:'0.72rem',marginBottom:14}}>{currentWeekStart}</div><div style={Object.assign({},cardS,{padding:'14px',marginBottom:12})}><div style={{display:'flex',justifyContent:'space-around'}}><div style={{textAlign:'center'}}><div style={{color:T.a1,fontFamily:'Playfair Display,serif',fontWeight:'bold',fontSize:'1.4rem'}}>{weekGlowTotal}</div><div style={{color:'rgba(255,179,217,.4)',fontSize:'0.6rem',textTransform:'uppercase'}}>Glow</div></div><div style={{textAlign:'center'}}><div style={{color:'#ff3d9a',fontFamily:'Playfair Display,serif',fontWeight:'bold',fontSize:'1.4rem'}}>{weekGrindTotal}</div><div style={{color:'rgba(255,179,217,.4)',fontSize:'0.6rem',textTransform:'uppercase'}}>Grind</div></div><div style={{textAlign:'center'}}><div style={{color:'#ffb347',fontFamily:'Playfair Display,serif',fontWeight:'bold',fontSize:'1.4rem'}}>{currentWeekData.perfectWeekApplied?'🏆':'—'}</div><div style={{color:'rgba(255,179,217,.4)',fontSize:'0.6rem',textTransform:'uppercase'}}>Perfect</div></div></div></div><div style={lblS}>Daily Breakdown</div>{currentWeekDates.map(function(dateStr){var dayCh=currentWeekData.challenges[dateStr]||[];var doneCh=dayCh.filter(function(c){return c.done;}).length;var d=new Date(dateStr);var dayId2=(['sun','mon','tue','wed','thu','fri','sat'])[d.getDay()];var sess2=SESSIONS[dayId2];var sessRequired=sess2&&(sess2.isGym||sess2.isRide);var sessDone=currentWeekData.trainingSessions[dayId2];var glowEarned=doneCh*GLOW_PER_CHALLENGE;var grindEarned=sessDone?(sess2.isGym?GRIND_GYM:GRIND_RIDE):0;var isFuture=new Date(dateStr)>new Date(today);return <div key={dateStr} style={Object.assign({},cardS,{padding:'11px 14px',marginBottom:8,opacity:isFuture?0.45:1})}><div style={{display:'flex',alignItems:'center',gap:10}}><div style={{minWidth:40}}><div style={{color:'#ffe8f5',fontFamily:'Playfair Display,serif',fontSize:'0.82rem',fontWeight:'bold'}}>{DAYS_SHORT[d.getDay()]}</div><div style={{color:'rgba(255,179,217,.3)',fontSize:'0.62rem'}}>{d.getDate()}/{d.getMonth()+1}</div></div><div style={{flex:1}}><div style={{display:'flex',gap:8,flexWrap:'wrap',marginBottom:3}}><span style={{fontSize:'0.72rem',color:doneCh>=3?T.a1:'rgba(255,179,217,.35)'}}>✨ {doneCh}</span>{sessRequired&&<span style={{fontSize:'0.72rem',color:sessDone?'#ff3d9a':'rgba(255,179,217,.35)'}}>💪 {sessDone?'✓':isFuture?'—':'✗'}</span>}{!sessRequired&&<span style={{fontSize:'0.72rem',color:'rgba(255,179,217,.25)'}}>rest</span>}</div></div><div style={{textAlign:'right'}}>{glowEarned>0&&<div style={{color:T.a1,fontSize:'0.68rem'}}>+{glowEarned}</div>}{grindEarned>0&&<div style={{color:'#ff3d9a',fontSize:'0.68rem'}}>+{grindEarned}</div>}</div></div></div>; })}</div>;
  }

  function renderRuleCard(rule, dis) {
    return <div key={rule.id} style={Object.assign({},cardS,{padding:'11px 13px',opacity:dis?0.42:1})}><div style={{display:'flex',alignItems:'center',gap:10}}><span style={{fontSize:'1.1rem'}}>{rule.emoji}</span><div style={{flex:1,minWidth:0}}><div style={{color:'#ffe8f5',fontFamily:'Playfair Display,serif',fontSize:'0.82rem',fontWeight:'bold',textDecoration:dis?'line-through':'none',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{rule.title}</div></div><button onClick={function(){setSt(function(p){var o=Object.assign({},p.rulesOvr);o[rule.id]=Object.assign({},o[rule.id]||{},{disabled:!dis});return Object.assign({},p,{rulesOvr:o});});}} style={{width:44,height:24,borderRadius:12,border:'none',cursor:'pointer',background:dis?'rgba(255,120,190,.18)':'linear-gradient(135deg,'+T.a1+','+T.a2+')',position:'relative',touchAction:'manipulation',flexShrink:0}}><div style={{position:'absolute',top:3,left:dis?3:23,width:18,height:18,borderRadius:'50%',background:'#fff',transition:'left .2s'}}/></button></div></div>;
  }
  function renderCustomRuleCard(rule, dis) {
    return <div key={rule.id} style={Object.assign({},cardS,{padding:'11px 13px',opacity:dis?0.42:1,border:'1px solid '+T.a1+'22'})}><div style={{display:'flex',alignItems:'center',gap:10}}><span>{rule.emoji}</span><div style={{flex:1,minWidth:0}}><div style={{color:'#ffe8f5',fontFamily:'Playfair Display,serif',fontSize:'0.82rem',fontWeight:'bold',textDecoration:dis?'line-through':'none'}}>{rule.title}</div></div><button onClick={function(){setSt(function(p){var o=Object.assign({},p.rulesOvr);o[rule.id]=Object.assign({},o[rule.id]||{},{disabled:!dis});return Object.assign({},p,{rulesOvr:o});});}} style={{width:44,height:24,borderRadius:12,border:'none',cursor:'pointer',background:dis?'rgba(255,120,190,.18)':'linear-gradient(135deg,'+T.a1+','+T.a2+')',position:'relative',touchAction:'manipulation',flexShrink:0,marginRight:6}}><div style={{position:'absolute',top:3,left:dis?3:23,width:18,height:18,borderRadius:'50%',background:'#fff',transition:'left .2s'}}/></button><button onClick={function(){if(window.confirm('Delete?'))setSt(function(p){return Object.assign({},p,{customRules:p.customRules.filter(function(x){return x.id!==rule.id;})});});}} style={{background:'transparent',border:'none',color:'rgba(255,100,100,.38)',cursor:'pointer',fontSize:'0.9rem',padding:'4px',touchAction:'manipulation'}}>Delete</button></div></div>;
  }
  function renderAdminRules() {
    var builtInRules = DAILY_RULES.map(function(rule){var dis=!!(st.rulesOvr[rule.id]&&st.rulesOvr[rule.id].disabled);return renderRuleCard(rule,dis);});
    var customRulesList = st.customRules.length>0 ? st.customRules.map(function(rule){var dis=!!(st.rulesOvr[rule.id]&&st.rulesOvr[rule.id].disabled);return renderCustomRuleCard(rule,dis);}) : null;
    var addRuleForm = <div style={Object.assign({},cardS,{padding:'16px',marginBottom:16})}><div style={{color:T.a1,fontFamily:'Playfair Display,serif',fontSize:'0.9rem',fontWeight:'bold',marginBottom:12}}>Add a Custom Rule</div><div style={{display:'grid',gridTemplateColumns:'60px 1fr',gap:9,marginBottom:10}}><div><div style={lblS}>Emoji</div><input value={st.ruleForm.emoji} onChange={function(e){setSt(function(p){return Object.assign({},p,{ruleForm:Object.assign({},p.ruleForm,{emoji:e.target.value})});});}} style={Object.assign({},inpS,{textAlign:'center',fontSize:'1.3rem',padding:'10px 6px'})} maxLength={2}/></div><div><div style={lblS}>Title</div><input value={st.ruleForm.title} onChange={function(e){setSt(function(p){return Object.assign({},p,{ruleForm:Object.assign({},p.ruleForm,{title:e.target.value}),ruleFormErr:''});});}} style={inpS}/></div></div><div style={{marginBottom:12}}><div style={lblS}>Description</div><textarea rows={2} value={st.ruleForm.description} onChange={function(e){setSt(function(p){return Object.assign({},p,{ruleForm:Object.assign({},p.ruleForm,{description:e.target.value})});});}} style={Object.assign({},inpS,{resize:'none',lineHeight:1.5})}/></div>{st.ruleFormErr&&<p style={{color:'#ff3d9a',fontSize:'0.82rem',marginBottom:10}}>{st.ruleFormErr}</p>}<button style={gbS} onClick={function(){if(!st.ruleForm.title.trim()){setSt(function(p){return Object.assign({},p,{ruleFormErr:'Add a title'});});return;}var nr={id:'cr-'+Date.now(),emoji:st.ruleForm.emoji,title:st.ruleForm.title.trim(),description:st.ruleForm.description.trim(),custom:true};setSt(function(p){return Object.assign({},p,{customRules:p.customRules.concat([nr]),ruleForm:{emoji:'✅',title:'',description:''},ruleFormErr:''});});}}>Add Rule ✅</button></div>;
    return <div>{addRuleForm}<div style={lblS}>Built-in Rules</div><div style={{display:"flex",flexDirection:"column",gap:7,marginTop:6,marginBottom:16}}>{builtInRules}</div>{customRulesList&&<div><div style={lblS}>Custom Rules</div><div style={{display:"flex",flexDirection:"column",gap:7,marginTop:6}}>{customRulesList}</div></div>}</div>;
  }

  function renderAdminNaughty() {
    return <div><div style={Object.assign({},cardS,{padding:'18px',marginBottom:14,border:'1px solid rgba(255,68,68,.22)'})}><div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:10}}><div><div style={{color:'#ff4444',fontFamily:'Playfair Display,serif',fontSize:'1rem',fontWeight:'bold',marginBottom:3}}>😈 Naughty Mode</div><div style={{color:'rgba(255,179,217,.4)',fontSize:'0.78rem',fontFamily:'Crimson Text,serif'}}>{naughtyActive?'Active today — resets at midnight':'Off — tap to activate for today'}</div></div><button onClick={function(){var na=!naughtyActive;setSt(function(p){return Object.assign({},p,{naughtyActive:na,naughtyDate:na?today:null});});}} style={{width:60,height:30,borderRadius:15,border:'none',cursor:'pointer',background:naughtyActive?'linear-gradient(135deg,#ff4444,#cc0000)':'rgba(255,68,68,.18)',position:'relative',touchAction:'manipulation'}}><div style={{position:'absolute',top:4,left:naughtyActive?33:4,width:22,height:22,borderRadius:'50%',background:'#fff',transition:'left .2s'}}/></button></div><div style={{color:'rgba(255,179,217,.4)',fontSize:'0.72rem',fontFamily:'Crimson Text,serif',lineHeight:1.5}}>Max 1 Naughty challenge per day. Level 2+ required. Resets at midnight.</div></div><div style={lblS}>Naughty Challenges</div><div style={{display:'flex',flexDirection:'column',gap:7,marginTop:6}}>{CHALLENGES.filter(function(c){return c.category==='Naughty';}).map(function(c){var ov=st.ovr[c.id]||{};var dis=!!ov.disabled;return <div key={c.id} style={Object.assign({},cardS,{padding:'10px 12px',opacity:dis?0.42:1})}><div style={{display:'flex',alignItems:'center',gap:8}}><span style={{fontSize:'0.95rem'}}>{c.emoji}</span><div style={{flex:1,minWidth:0}}><div style={{color:'#ffe8f5',fontFamily:'Playfair Display,serif',fontSize:'0.8rem',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap',textDecoration:dis?'line-through':'none'}}>{c.title}</div><div style={{color:'rgba(255,179,217,.3)',fontSize:'0.58rem',marginTop:2}}>Level {c.level}</div></div><button onClick={function(){setOv(c.id,{disabled:!dis});}} style={{width:44,height:24,borderRadius:12,border:'none',cursor:'pointer',background:dis?'rgba(255,120,190,.18)':'linear-gradient(135deg,#ff4444,#cc0000)',position:'relative',touchAction:'manipulation',flexShrink:0}}><div style={{position:'absolute',top:3,left:dis?3:23,width:18,height:18,borderRadius:'50%',background:'#fff',transition:'left .2s'}}/></button></div></div>; })}</div></div>;
  }

  function renderRewardCard(r) {
    var isGlow=r.pointType==='glow';var ptColor=isGlow?T.a1:'#ff3d9a';var ptLabel=isGlow?'✨ Glow Points':'💪 Grind Points';var minLvl=r.minLevel||0;var lvlReq=minLvl>0?GLOW_LEVELS.find(function(l){return l.level===minLvl;}):null;var locked=!!(lvlReq&&glowLvi.level<minLvl);var balance=isGlow?st.glowPoints:st.grindPoints;var canAfford=balance>=r.cost;
    return <div key={r.id} style={Object.assign({},cardS,{padding:'13px 14px',opacity:r.redeemed?0.5:1})}><div style={{display:'flex',alignItems:'center',gap:10}}><span style={{fontSize:'1.4rem'}}>{r.emoji}</span><div style={{flex:1,minWidth:0}}><div style={{color:'#ffe8f5',fontFamily:'Playfair Display,serif',fontSize:'0.86rem',fontWeight:'bold',textDecoration:r.redeemed?'line-through':'none'}}>{r.title}</div><div style={{display:'flex',gap:8,alignItems:'center',marginTop:3,flexWrap:'wrap'}}><span style={{color:ptColor,fontSize:'0.68rem',fontWeight:'bold'}}>{r.cost} {isGlow?'✨':'💪'}</span>{r.minLevel>0&&<span style={{color:'rgba(255,179,217,.4)',fontSize:'0.62rem'}}>Level {r.minLevel}+</span>}{r.redeemed&&<span style={{color:'rgba(255,179,217,.35)',fontSize:'0.64rem',fontStyle:'italic'}}>redeemed</span>}</div></div><div style={{display:'flex',gap:6}}>{r.redeemed&&<button onClick={function(){setSt(function(p){return Object.assign({},p,{rewards:p.rewards.map(function(x){return x.id===r.id?Object.assign({},x,{redeemed:false,redeemedAt:null}):x;})});});}} style={{background:'transparent',border:'1px solid rgba(255,120,190,.2)',borderRadius:8,color:'rgba(255,179,217,.4)',cursor:'pointer',fontSize:'0.64rem',padding:'4px 8px',touchAction:'manipulation'}}>Restore</button>}<button onClick={function(){if(window.confirm('Delete?'))setSt(function(p){return Object.assign({},p,{rewards:p.rewards.filter(function(x){return x.id!==r.id;})});});}} style={{background:'transparent',border:'none',color:'rgba(255,100,100,.34)',cursor:'pointer',fontSize:'0.8rem',padding:'4px',touchAction:'manipulation'}}>✕</button></div></div>{!locked&&!r.redeemed&&<div style={{marginBottom:8}}><div style={{height:4,background:'rgba(255,255,255,.05)',borderRadius:4,overflow:'hidden'}}><div style={{height:'100%',width:Math.min(100,(balance/r.cost)*100)+'%',background:'linear-gradient(90deg,'+ptColor+','+ptColor+'88)',borderRadius:4}}/></div>{!canAfford&&<div style={{color:'rgba(255,179,217,.3)',fontSize:'0.62rem',marginTop:3}}>{r.cost-balance} more {isGlow?'Glow':'Grind'} Pts needed</div>}</div>}{locked&&lvlReq&&<div style={{marginBottom:8}}><div style={{color:'rgba(255,179,217,.4)',fontSize:'0.7rem',marginBottom:5}}>🔒 Unlocks at {lvlReq.emoji} {lvlReq.title}</div><div style={{height:4,background:'rgba(255,255,255,.05)',borderRadius:4,overflow:'hidden'}}><div style={{height:'100%',width:Math.min(100,((st.glowPoints/Math.max(1,lvlReq.threshold))*100))+'%',background:'linear-gradient(90deg,'+T.a1+'66,'+T.a2+'66)',borderRadius:4}}/></div></div>}{r.redeemed?<div style={{fontSize:'0.75rem',color:'rgba(255,179,217,.35)',fontStyle:'italic'}}>Redeemed ✓</div>:!locked&&<button onClick={function(){if(!canAfford)return;if(!window.confirm('Redeem?'))return;setSt(function(p){var nr2=p.rewards.map(function(x){return x.id===r.id?Object.assign({},x,{redeemed:true,redeemedAt:new Date().toISOString()}):x;});var ng=isGlow?Math.max(0,p.glowPoints-r.cost):p.glowPoints;var ngr=!isGlow?Math.max(0,p.grindPoints-r.cost):p.grindPoints;return Object.assign({},p,{rewards:nr2,glowPoints:ng,grindPoints:ngr});});}} style={{background:canAfford?'linear-gradient(135deg,'+T.a1+','+T.a2+')':'rgba(255,255,255,.06)',border:'none',borderRadius:20,color:canAfford?'#fff':'rgba(255,179,217,.25)',fontFamily:'Playfair Display,serif',fontSize:'0.78rem',fontWeight:'bold',padding:'7px 16px',cursor:canAfford?'pointer':'not-allowed',touchAction:'manipulation'}}>{canAfford?'Redeem 💖':'Not enough points'}</button>}</div>;
  }
  function renderAdminRewards() {
    var addForm = <div style={Object.assign({},cardS,{padding:'16px',marginBottom:16})}><div style={{color:T.a1,fontFamily:'Playfair Display,serif',fontSize:'0.9rem',fontWeight:'bold',marginBottom:12}}>Add a Reward</div><div style={{display:'grid',gridTemplateColumns:'60px 1fr',gap:9,marginBottom:10}}><div><div style={lblS}>Emoji</div><input value={st.rewardForm.emoji} onChange={function(e){setSt(function(p){return Object.assign({},p,{rewardForm:Object.assign({},p.rewardForm,{emoji:e.target.value})});});}} style={Object.assign({},inpS,{textAlign:'center',fontSize:'1.3rem',padding:'10px 6px'})} maxLength={2}/></div><div><div style={lblS}>Title</div><input value={st.rewardForm.title} onChange={function(e){setSt(function(p){return Object.assign({},p,{rewardForm:Object.assign({},p.rewardForm,{title:e.target.value}),rewardFormErr:''});});}} style={inpS}/></div></div><div style={{marginBottom:10}}><div style={lblS}>Description</div><input value={st.rewardForm.desc} onChange={function(e){setSt(function(p){return Object.assign({},p,{rewardForm:Object.assign({},p.rewardForm,{desc:e.target.value})});});}} style={inpS}/></div><div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:9,marginBottom:12}}><div><div style={lblS}>Points</div><div style={{display:'flex',gap:5}}>{[['glow','✨'],['grind','💪']].map(function(pair){var sel=st.rewardForm.pointType===pair[0];return <button key={pair[0]} onClick={function(){setSt(function(p){return Object.assign({},p,{rewardForm:Object.assign({},p.rewardForm,{pointType:pair[0]})});});}} style={{flex:1,padding:'10px 4px',borderRadius:10,border:'1px solid '+(sel?T.a1+'66':'rgba(255,120,190,.2)'),background:sel?T.a1+'18':'transparent',color:sel?T.a1:'rgba(255,179,217,.4)',cursor:'pointer',fontSize:'0.76rem',touchAction:'manipulation'}}>{pair[1]}</button>; })}</div></div><div><div style={lblS}>Cost</div><input type='number' min='1' value={st.rewardForm.cost} onChange={function(e){setSt(function(p){return Object.assign({},p,{rewardForm:Object.assign({},p.rewardForm,{cost:Number(e.target.value)})});});}} style={inpS}/></div><div><div style={lblS}>Min Level</div><select value={st.rewardForm.minLevel} onChange={function(e){setSt(function(p){return Object.assign({},p,{rewardForm:Object.assign({},p.rewardForm,{minLevel:Number(e.target.value)})});});}} style={inpS}><option value={0}>Any</option>{GLOW_LEVELS.map(function(l){return <option key={l.level} value={l.level}>{l.emoji} {l.title}</option>;})}</select></div></div>{st.rewardFormErr&&<p style={{color:'#ff3d9a',fontSize:'0.82rem',marginBottom:10}}>{st.rewardFormErr}</p>}<button style={gbS} onClick={function(){if(!st.rewardForm.title.trim()){setSt(function(p){return Object.assign({},p,{rewardFormErr:'Add a title'});});return;}var nr={id:'r-'+Date.now(),emoji:st.rewardForm.emoji,title:st.rewardForm.title.trim(),desc:st.rewardForm.desc.trim(),pointType:st.rewardForm.pointType,cost:Number(st.rewardForm.cost),minLevel:Number(st.rewardForm.minLevel)||0,redeemed:false,createdAt:new Date().toISOString()};setSt(function(p){return Object.assign({},p,{rewards:p.rewards.concat([nr]),rewardForm:{emoji:'🎁',title:'',desc:'',pointType:'glow',cost:50,minLevel:0},rewardFormErr:''});});}}>Add Reward 🎁</button></div>;
    var list = st.rewards.length>0 ? st.rewards.map(renderRewardCard) : null;
    return <div><p style={{color:'rgba(255,179,217,.4)',fontSize:'0.78rem',fontFamily:'Crimson Text,serif',marginBottom:14,lineHeight:1.5}}>Add rewards Rebecca can redeem with her points.</p>{addForm}{list&&<div><div style={lblS}>Current Rewards</div><div style={{display:'flex',flexDirection:'column',gap:9,marginTop:6}}>{list}</div></div>}</div>;
  }

  function renderAdminSettings() {
    var themeCard = <div style={Object.assign({},cardS,{padding:'14px',marginBottom:11})}><div style={lblS}>Theme</div><div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8,marginTop:8}}>{Object.keys(THEMES).map(function(key){var t=THEMES[key];var sel=st.theme===key;return <button key={key} onClick={function(){setSt(function(p){return Object.assign({},p,{theme:key});});}} style={{background:sel?'linear-gradient(135deg,'+t.a1+'22,'+t.a2+'22)':'rgba(255,255,255,.03)',border:'1px solid '+(sel?t.a1+'66':'rgba(255,120,190,.13)'),borderRadius:12,padding:'12px 9px',cursor:'pointer',touchAction:'manipulation',textAlign:'center'}}><div style={{height:10,borderRadius:4,background:'linear-gradient(90deg,'+t.a1+','+t.a2+')',marginBottom:7}}/><div style={{color:sel?t.a1:'rgba(255,179,217,.46)',fontFamily:'Playfair Display,serif',fontSize:'0.78rem',fontWeight:sel?'bold':'normal'}}>{t.name}</div></button>; })}</div></div>;
    var notifCard = <div style={Object.assign({},cardS,{padding:'14px',marginBottom:11})}><div style={lblS}>Notifications</div><div style={{marginBottom:10}}><div style={{color:'rgba(255,179,217,.4)',fontSize:'0.72rem',marginBottom:5}}>Challenge nudge</div><input type='time' value={st.challengeNudgeTime} onChange={function(e){setSt(function(p){return Object.assign({},p,{challengeNudgeTime:e.target.value});});}} style={Object.assign({},inpS,{colorScheme:'dark'})}/></div><div><div style={{color:'rgba(255,179,217,.4)',fontSize:'0.72rem',marginBottom:5}}>Training nudge</div><input type='time' value={st.trainingNudgeTime} onChange={function(e){setSt(function(p){return Object.assign({},p,{trainingNudgeTime:e.target.value});});}} style={Object.assign({},inpS,{colorScheme:'dark'})}/></div></div>;
    var milestoneCard = <div style={Object.assign({},cardS,{padding:'14px',marginBottom:11})}><div style={lblS}>Milestone Messages</div>{Object.keys(MILESTONE_MESSAGES).map(function(key){var base=MILESTONE_MESSAGES[key];var custom=(st.milestoneMessages||{})[key]||{};return <div key={key} style={{marginBottom:12}}><div style={{color:'rgba(255,179,217,.5)',fontSize:'0.7rem',marginBottom:4}}>{base.emoji} {base.title}</div><input placeholder={base.msg.substring(0,50)+'...'} value={custom.msg||''} onChange={function(e){setSt(function(p){var mm=Object.assign({},p.milestoneMessages||{});mm[key]=Object.assign({},mm[key]||{},{msg:e.target.value||undefined});return Object.assign({},p,{milestoneMessages:mm});});}} style={Object.assign({},inpS,{fontSize:'0.82rem'})}/></div>; })}</div>;
    var visitCard = <div style={Object.assign({},cardS,{padding:'14px',marginBottom:11})}><div style={lblS}>Next Visit Date</div><div style={{color:'rgba(255,179,217,.4)',fontSize:'0.74rem',fontFamily:'Crimson Text,serif',marginBottom:10,lineHeight:1.5}}>Set the date of your next visit. Rebecca will see the countdown at the top of her screen.</div><input type='date' value={st.nextVisit||''} onChange={function(e){setSt(function(p){return Object.assign({},p,{nextVisit:e.target.value||null});});}} style={Object.assign({},inpS,{colorScheme:'dark',marginBottom:0})}/>{st.nextVisit&&<button onClick={function(){setSt(function(p){return Object.assign({},p,{nextVisit:null});});}} style={{background:'transparent',border:'none',color:'rgba(255,100,100,.4)',cursor:'pointer',fontSize:'0.72rem',marginTop:8,padding:0,display:'block',fontFamily:'Crimson Text,serif',touchAction:'manipulation'}}>Clear date</button>}</div>;
    var onboardCard = <div style={Object.assign({},cardS,{padding:'14px',marginBottom:11})}><div style={lblS}>Onboarding</div><button onClick={function(){setOnboardDone(false);setOnboardSlide(0);}} style={Object.assign({},sbS(false),{width:'100%',padding:'11px'})}>Reset Onboarding</button></div>;
    var dangerCard = <div style={{borderTop:'1px solid rgba(255,120,190,.1)',paddingTop:16,marginBottom:10}}><div style={lblS}>Danger Zone</div><button onClick={function(){if(window.confirm('Reset ALL progress?')){setSt(function(p){return Object.assign({},p,{glowPoints:0,grindPoints:0,seen:[],seenDates:{},ch:[],lgd:null,hist:{},streak:0,lsd:null,graceUsedWeek:null,cats:{},earned:[],wplan:{},weeklyData:{},perfectWeeks:0,pb:{bestGlowWeek:0,bestGrindWeek:0,bestPerfectStreak:0,mostChallengesDay:0},rulesChecked:{},rulesStreak:0,rulesLastDate:null,perfectDays:0,herMessages:[],shownMilestones:[]});});setTChecked({});setRidesDone({});}}} style={{background:'transparent',border:'1px solid rgba(255,60,60,.22)',color:'rgba(255,100,100,.4)',borderRadius:12,padding:11,cursor:'pointer',fontSize:'0.8rem',width:'100%',marginBottom:8,touchAction:'manipulation'}}>Reset All Progress</button></div>;
    return <div>{themeCard}{visitCard}{notifCard}{milestoneCard}{onboardCard}{dangerCard}<button onClick={function(){setSt(function(p){return Object.assign({},p,{unlocked:false,view:'her'});});}} style={Object.assign({},sbS(false),{width:'100%',padding:'11px'})}>Lock and Return</button></div>;
  }


  function renderGymSession() {
    var warmupSection = SESSIONS[tDay].warmup?<div style={{marginBottom:14}}><div style={{fontSize:9,letterSpacing:3,color:'#FF85AD',fontWeight:700,marginBottom:8,paddingLeft:2}}>WARMUP — 10 MINS</div><div style={{background:'rgba(255,133,173,0.06)',border:'1px solid rgba(255,133,173,0.15)',borderRadius:14}}>{SESSIONS[tDay].warmup.map(function(item,i){var wk=tDay+'-w-'+i;var wdone=!!tWarmup[wk];return <div key={i} className='tappable' onClick={function(){setTWarmup(function(p){return Object.assign({},p,{[wk]:!p[wk]});});}} style={{display:'flex',alignItems:'center',gap:12,padding:'10px 14px',borderBottom:i<SESSIONS[tDay].warmup.length-1?'1px solid rgba(255,255,255,0.04)':'none'}}><div style={{width:18,height:18,borderRadius:'50%',flexShrink:0,border:'1.5px solid '+(wdone?'#FF85AD':'rgba(255,133,173,0.3)'),background:wdone?'rgba(255,133,173,0.25)':'transparent',display:'flex',alignItems:'center',justifyContent:'center',fontSize:9,color:'#FF85AD'}}>{wdone?'✓':''}</div><div style={{fontSize:12,fontWeight:300,lineHeight:1.3,color:wdone?'#4A2535':'#C49AAA',textDecoration:wdone?'line-through':'none'}}>{item}</div></div>; })}</div></div>:null;
    var exList = <div style={{display:'flex',flexDirection:'column',gap:7}}>{SESSIONS[tDay].exercises.map(function(ex,i){var ek=tDay+'-'+i;var edone=!!tChecked[ek];return <div key={i} className='tappable' onClick={function(){setTChecked(function(p){return Object.assign({},p,{[ek]:!p[ek]});});}} style={{background:edone?'rgba(255,45,120,0.07)':'rgba(255,255,255,0.04)',border:edone?'1px solid rgba(255,45,120,0.28)':'1px solid rgba(255,255,255,0.07)',borderRadius:14,padding:'13px 14px',display:'flex',alignItems:'center',gap:12}}><div style={{width:27,height:27,borderRadius:'50%',flexShrink:0,background:edone?'linear-gradient(135deg,#FF2D78,#CC0044)':'transparent',border:edone?'none':'2px solid rgba(255,45,120,0.38)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:12,color:'white',transition:'all 0.22s ease'}}>{edone?'✓':''}</div><div style={{flex:1,minWidth:0}}><div style={{fontSize:13,fontWeight:600,color:edone?'#4A2535':'white',textDecoration:edone?'line-through':'none',marginBottom:2,lineHeight:1.2}}>{ex.name}</div><div style={{fontSize:11,color:edone?'#3A1A28':'#6A4055',fontWeight:300,lineHeight:1.3}}>{ex.note}</div></div><div style={{background:edone?'rgba(255,45,120,0.12)':'rgba(255,45,120,0.1)',borderRadius:8,padding:'4px 9px',fontSize:10,fontWeight:700,color:edone?'#4A2535':'#FF2D78',flexShrink:0,whiteSpace:'nowrap'}}>{ex.sets}</div></div>; })}</div>;
    var kneeRules = SESSIONS[tDay].showKneeRules?<div style={{marginTop:12,background:'rgba(255,45,120,0.05)',border:'1px solid rgba(255,45,120,0.14)',borderRadius:14,padding:'13px 14px'}}><div style={{fontSize:8,letterSpacing:3,color:'#FF2D78',fontWeight:700,marginBottom:10}}>KNEE RULES</div><div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'7px 10px'}}>{KNEE_RULES.map(function(rule,i){return <div key={i} style={{fontSize:10,color:'#6A4055',fontWeight:300,lineHeight:1.4,display:'flex',gap:6,alignItems:'flex-start'}}><span style={{color:'#FF2D78',flexShrink:0,marginTop:1}}>—</span>{rule}</div>; })}</div></div>:null;
    var progToggle = <div className='tappable' onClick={function(){setShowProg(function(p){return !p;});}} style={{marginTop:12,background:'rgba(255,255,255,0.04)',border:'1px solid rgba(255,255,255,0.08)',borderRadius:14,padding:'12px 14px',display:'flex',justifyContent:'space-between',alignItems:'center'}}><div style={{fontSize:10,letterSpacing:2,color:'#7A5068',fontWeight:600}}>PROGRESSION PLAN</div><div style={{fontSize:10,color:'#FF2D78',transform:showProg?'rotate(180deg)':'rotate(0)',transition:'transform 0.2s ease'}}>▼</div></div>;
    var progList = showProg?<div style={{marginTop:6,display:'flex',flexDirection:'column',gap:6}}>{PROGRESSION.map(function(p,i){return <div key={i} style={{background:'rgba(255,255,255,0.04)',border:'1px solid rgba(255,255,255,0.07)',borderRadius:12,padding:'11px 14px',display:'flex',gap:12,alignItems:'flex-start'}}><div style={{fontSize:9,fontWeight:700,color:'#FF2D78',letterSpacing:1,flexShrink:0,marginTop:2}}>{p.phase}</div><div style={{fontSize:12,color:'#9A7080',fontWeight:300,lineHeight:1.4}}>{p.desc}</div></div>; })}</div>:null;
    return <div>{warmupSection}<div style={{fontSize:9,letterSpacing:3,color:"#FF2D78",fontWeight:700,marginBottom:8,paddingLeft:2}}>EXERCISES</div>{exList}{kneeRules}{progToggle}{progList}</div>;
  }

  function renderNutritionMeals() {
    var nutMeals2 = nutView==='weekend'?WEEKEND_MEALS:WEEKDAY_MEALS;
    return <div style={{padding:'0 20px'}}><div style={{display:'flex',flexDirection:'column',gap:9}}>{nutMeals2.map(function(meal){var done=!!nutChecked[meal.id];var expanded=expandedMeal===meal.id;return <div key={meal.id} style={{background:done?'rgba(255,45,120,0.06)':'rgba(255,255,255,0.04)',border:done?'1px solid rgba(255,45,120,0.24)':'1px solid rgba(255,255,255,0.07)',borderRadius:16,overflow:'hidden'}}><div className='tappable' onClick={function(){setExpandedMeal(function(p){return p===meal.id?null:meal.id;});}} style={{padding:'13px 14px'}}><div style={{display:'flex',alignItems:'center',gap:11}}><div onClick={function(e){e.stopPropagation();setNutChecked(function(p){var wasChecked=!!p[meal.id];var nc=Object.assign({},p,{[meal.id]:!wasChecked});if(!wasChecked){setSt(function(s){var k='nutDay-'+today;return s.nutDaysLogged&&s.nutDaysLogged[k]?s:Object.assign({},s,{nutDaysLogged:Object.assign({},s.nutDaysLogged||{},{[k]:true})});});}return nc;});}} style={{width:28,height:28,borderRadius:'50%',flexShrink:0,background:done?'linear-gradient(135deg,#FF2D78,#CC0044)':'transparent',border:done?'none':'2px solid rgba(255,45,120,0.34)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:12,color:'white',cursor:'pointer',transition:'all 0.22s ease'}}>{done?'✓':''}</div><div style={{fontSize:20,flexShrink:0}}>{meal.emoji}</div><div style={{flex:1,minWidth:0}}><div style={{display:'flex',alignItems:'center',gap:6,marginBottom:2}}><div style={{fontSize:7,letterSpacing:2,fontWeight:700,color:done?'#4A2535':'#FF85AD'}}>{meal.time}</div>{meal.flexible&&<div style={{fontSize:7,color:'#4A2535',background:'rgba(255,255,255,0.05)',padding:'1px 6px',borderRadius:4}}>FLEXIBLE</div>}</div><div style={{fontSize:13,fontWeight:600,color:done?'#4A2535':'white',textDecoration:done?'line-through':'none',lineHeight:1.2}}>{meal.name}</div></div><div style={{textAlign:'right',flexShrink:0}}><div style={{fontSize:15,fontWeight:700,color:done?'#4A2535':'#FF2D78'}}>{meal.kcal}</div><div style={{fontSize:8,color:'#4A2535'}}>kcal</div></div><div style={{fontSize:9,color:'#4A2535',transform:expanded?'rotate(180deg)':'rotate(0)',transition:'transform 0.2s ease'}}>▼</div></div><div style={{display:'flex',gap:5,marginTop:8,paddingLeft:39}}>{[{l:'P',v:meal.protein,c:'#FF2D78'},{l:'C',v:meal.carbs,c:'#FF85AD'},{l:'F',v:meal.fat,c:'#FFB347'},{l:'Fi',v:meal.fibre,c:'#A8D8A8'}].map(function(m){return <div key={m.l} style={{background:'rgba(255,255,255,0.05)',borderRadius:6,padding:'2px 7px',fontSize:9,fontWeight:600,color:done?'#3A1A28':m.c}}>{m.l} {m.v}g</div>; })}</div></div>{expanded&&<div style={{borderTop:'1px solid rgba(255,255,255,0.05)',padding:'11px 14px 13px'}}>{meal.items.map(function(item,i){return <div key={i} style={{display:'flex',gap:10,padding:'5px 0',borderBottom:i<meal.items.length-1?'1px solid rgba(255,255,255,0.04)':'none'}}><div style={{fontSize:11,color:'#FF85AD',fontWeight:600,minWidth:48,flexShrink:0}}>{item.amount}</div><div style={{fontSize:11,color:'#9A7080',fontWeight:300}}>{item.ingredient}</div></div>; })}<div style={{marginTop:10,background:'rgba(255,45,120,0.07)',borderRadius:8,padding:'8px 11px',fontSize:11,color:'#FF85AD',fontWeight:300,lineHeight:1.4,display:'flex',gap:7}}><span style={{flexShrink:0}}>—</span><span>{meal.note}</span></div></div>}</div>; })}<div style={{marginTop:12,background:'rgba(255,255,255,0.03)',border:'1px solid rgba(255,255,255,0.06)',borderRadius:14,padding:'11px 14px',display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:4}}><div style={{fontSize:11,color:'#5A3545',fontWeight:300}}>Target: {nutView==='weekend'?'~2,660':'2,500-2,600'} kcal</div><div style={{display:'flex',alignItems:'baseline',gap:4}}><div style={{fontSize:20,fontWeight:700,color:nutTotals.kcal>=2400?'#FF2D78':'#F5E8EF'}}>{nutTotals.kcal}</div><div style={{fontSize:9,color:'#4A2535'}}>logged</div></div></div></div></div>;
  }

  function renderRewardsList() {
    if (st.rewards.length===0) return <div style={{textAlign:'center',padding:'50px 20px',color:'rgba(255,179,217,.26)',fontStyle:'italic',fontFamily:'Crimson Text,serif',lineHeight:1.7}}>No rewards yet 🌸<br/>Sam will add some for you to unlock</div>;
    return <div style={{display:"flex",flexDirection:"column",gap:12}}>{st.rewards.map(renderRewardCard)}</div>;
  }
    ...
    <div style={{minHeight:'100vh',background:T.bg,fontFamily:'Palatino Linotype,Palatino,Georgia,serif',position:'relative',overflow:'hidden'}}>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0;-webkit-tap-highlight-color:transparent}
        ...
        input:focus,textarea:focus,select:focus{border-color:rgba(255,120,190,.5)!important;box-shadow:0 0 0 3px rgba(255,60,150,.08)!important;}
        @keyframes glow{0%,100%{box-shadow:0 0 20px rgba(255,61,154,.2)}50%{box-shadow:0 0 40px rgba(255,61,154,.4)}}
        @keyframes slideUp{from{transform:translateY(30px);opacity:0}to{transform:translateY(0);opacity:1}}
      `}</style>

      {splash && <div style={{position:'fixed',inset:0,background:T.bg,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',zIndex:999,animation:'splFade .5s 1.8s ease-out forwards'}}><div style={{fontSize:'4rem',marginBottom:16,animation:'pulse 1.5s ease-in-out infinite'}}>💖</div><div style={{fontFamily:'Playfair Display,serif',fontWeight:900,fontSize:'2.6rem',background:'linear-gradient(135deg,'+T.a1+',#ffcce8,'+T.a2+')',backgroundSize:'200%',WebkitBackgroundClip:'text',WebkitTextFillColor:'transparent',animation:'shimmer 2s ease infinite'}}>Glow Up</div><div style={{color:'rgba(255,179,217,.45)',fontStyle:'italic',marginTop:8}}>your daily challenges</div></div>}
      {!splash && !onboardDone && <div style={{position:'fixed',inset:0,background:T.bg,zIndex:990,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',padding:'30px 24px'}}><div style={{maxWidth:400,width:'100%',textAlign:'center'}}><div style={{fontSize:'4rem',marginBottom:20,animation:'pulse 2s ease-in-out infinite'}}>{ONBOARDING_SLIDES[onboardSlide].emoji}</div><h2 style={{fontFamily:'Playfair Display,serif',fontWeight:900,fontSize:'1.8rem',background:'linear-gradient(135deg,'+T.a1+','+T.a2+')',WebkitBackgroundClip:'text',WebkitTextFillColor:'transparent',marginBottom:16,lineHeight:1.2}}>{ONBOARDING_SLIDES[onboardSlide].title}</h2><p style={{color:'rgba(255,179,217,.7)',fontFamily:'Crimson Text,serif',fontSize:'1.05rem',lineHeight:1.75,marginBottom:32}}>{ONBOARDING_SLIDES[onboardSlide].body}</p><div style={{display:'flex',justifyContent:'center',gap:8,marginBottom:28}}>{ONBOARDING_SLIDES.map(function(_,i){return <div key={i} style={{width:8,height:8,borderRadius:'50%',background:i===onboardSlide?T.a1:'rgba(255,120,190,.2)',transition:'background .3s'}}/>;})}</div><button style={gbS} onClick={function(){if(onboardSlide<ONBOARDING_SLIDES.length-1)setOnboardSlide(function(s){return s+1;});else setOnboardDone(true);}}>{ONBOARDING_SLIDES[onboardSlide].btn}</button></div></div>}

      <div style={{position:'fixed',inset:0,pointerEvents:'none',overflow:'hidden'}}>{[{t:'-8%',l:'-8%',s:'65vw',c:T.a1+'18'},{t:'38%',r:'-12%',s:'55vw',c:T.a2+'14'},{b:'-8%',l:'15%',s:'60vw',c:T.a1+'0f'}].map(function(b,i){return <div key={i} style={{position:'absolute',top:b.t,left:b.l,right:b.r,bottom:b.b,width:b.s,height:b.s,borderRadius:'50%',background:'radial-gradient(circle,'+b.c+',transparent 70%)',animation:'blob '+(8+i*1.5)+'s '+(i*2)+'s ease-in-out infinite'}}/>;})}</div>
      ...
      {milestone && <OVL onClose={function(){setMilestone(null);}}><div style={{background:'linear-gradient(135deg,rgba(22,0,15,.98),rgba(38,0,24,.98))',border:'1px solid '+T.a1+'66',borderRadius:26,padding:'44px 32px',textAlign:'center',animation:'milePop .6s ease-out'}}><div style={{fontSize:'3.5rem',marginBottom:14}}>{milestone.emoji}</div><div style={{color:'rgba(255,179,217,.45)',fontSize:'0.7rem',letterSpacing:'0.2em',textTransform:'uppercase',marginBottom:8}}>Milestone</div><div style={{fontFamily:'Playfair Display,serif',fontWeight:900,fontSize:'1.8rem',background:grad,WebkitBackgroundClip:'text',WebkitTextFillColor:'transparent',marginBottom:16,lineHeight:1.2}}>{milestone.title}</div><p style={{color:'rgba(255,179,217,.7)',fontFamily:'Crimson Text,serif',fontSize:'0.98rem',lineHeight:1.7,marginBottom:24}}>{milestone.msg}</p><button style={gbS} onClick={function(){setMilestone(null);}}>Thank you 💖</button></div></OVL>}
      {showPerfectWeek && <OVL onClose={function(){setShowPerfectWeek(false);}}><div style={{background:'linear-gradient(135deg,rgba(22,0,15,.97),rgba(38,0,24,.97))',border:'1px solid '+T.a1+'66',borderRadius:26,padding:'44px 32px',textAlign:'center',animation:'milePop .6s ease-out'}}><div style={{fontSize:'3.5rem',marginBottom:12}}>🏆</div><div style={{color:'rgba(255,179,217,.5)',fontSize:'0.7rem',letterSpacing:'0.2em',textTransform:'uppercase',marginBottom:8}}>Perfect Week</div><div style={{fontFamily:'Playfair Display,serif',fontWeight:900,fontSize:'1.9rem',background:grad,WebkitBackgroundClip:'text',WebkitTextFillColor:'transparent',marginBottom:10}}>1.5x Bonus!</div><p style={{color:'rgba(255,179,217,.6)',fontFamily:'Crimson Text,serif',fontSize:'0.95rem',lineHeight:1.6}}>Every challenge every session this week. 50% bonus applied. 💖</p></div></OVL>}
      ...
      <div style={{position:'relative',zIndex:1,maxWidth:520,margin:'0 auto',padding:'0 0 90px'}}>
        ...
        {tab==='home' && <div style={{padding:'0 15px'}}>
          ...
          {st.view==='her' && <div style={{animation:'up .4s ease-out'}}>
            ...
            {st.htab==='Today' && renderHerToday()}
            {st.htab==='Stats' && renderHerStats()}
            {st.htab==='History' && renderHerHistory()}
          </div>}
          {st.view==='admin' && st.unlocked && <div style={{animation:'up .4s ease-out'}}>
            ...
            {st.atab==='Today' && renderAdminToday()}
            {st.atab==='Challenges' && renderAdminChallenges()}
            {st.atab==='Planner' && renderAdminPlanner()}
            {st.atab==='Add New' && renderAdminAddNew()}
            {st.atab==='Bulk Import' && renderAdminBulk()}
            {st.atab==='Week' && renderAdminWeek()}
            {st.atab==='Rules' && renderAdminRules()}
            {st.atab==='Naughty' && renderAdminNaughty()}
            {st.atab==='Rewards' && renderAdminRewards()}
            {st.atab==='Settings' && renderAdminSettings()}
          </div>}
        </div>}

        {tab==='training' && <div style={{fontFamily:'Poppins,sans-serif',color:'#F5E8EF',animation:'up .3s ease-out'}}>
          ...
          <div style={{display:'flex',gap:5,padding:'0 22px 18px',overflowX:'auto'}}>
            {TRAINING_DAYS.map(function(d){var active=d.id===tDay;var isToday=d.id===todayDayId;var dotCol=d.type==='gym'?'#FF2D78':d.type==='ride'?'#FF85AD':'#3A2030';var done=isSessionDone(d.id);return <div key={d.id} className='tappable' onClick={function(){setTDay(d.id);setShowProg(false);}} style={{flexShrink:0,width:46,borderRadius:12,padding:'10px 0',textAlign:'center',background:active?'linear-gradient(155deg,#FF2D78,#CC0044)':done?'rgba(255,45,120,0.12)':isToday?'rgba(255,45,120,0.08)':'rgba(255,255,255,0.04)',border:active?'1.5px solid #FF2D78':done?'1.5px solid rgba(255,45,120,0.4)':isToday?'1.5px solid rgba(255,45,120,0.2)':'1.5px solid rgba(255,255,255,0.07)',boxShadow:active?'0 3px 16px rgba(255,45,120,0.38)':'none'}}><div style={{fontSize:8,fontWeight:700,letterSpacing:1,color:active?'rgba(255,255,255,0.65)':'#5A3545',marginBottom:4}}>{d.short}</div><div style={{width:7,height:7,borderRadius:'50%',background:active?'white':done?'#FF2D78':dotCol,margin:'0 auto 4px',opacity:active?0.9:0.6}}/><div style={{fontSize:7,color:active?'white':done?'#FF85AD':'#4A2535',fontWeight:active?600:400}}>{done?'DONE':d.type==='gym'?'GYM':d.type==='ride'?'RIDE':'REST'}</div></div>;})}
          </div>
          <div style={{padding:'0 20px'}} key={tDay}>
            <div style={{background:'rgba(255,45,120,0.1)',border:'1px solid rgba(255,45,120,0.22)',borderRadius:18,padding:'18px 18px 14px',marginBottom:12}}>
              ...
              {tTotalExs>0&&<div style={{marginTop:10}}><div style={{display:'flex',justifyContent:'space-between',marginBottom:5}}><div style={{fontSize:10,color:'#7A5068'}}>{tDoneExs}/{tTotalExs} exercises</div><div style={{fontSize:10,color:'#FF2D78',fontWeight:600}}>{Math.round(tProgress*100)}%</div></div><div style={{height:4,borderRadius:2,background:'rgba(255,255,255,0.07)'}}><div style={{height:'100%',borderRadius:2,width:(tProgress*100)+'%',background:'linear-gradient(90deg,#FF2D78,#FF85AD)',transition:'width 0.4s ease'}}/></div></div>}
            </div>
        ...
        {tab==='nutrition' && <div style={{fontFamily:'Poppins,sans-serif',color:'#F5E8EF',animation:'up .3s ease-out'}}>
          <div style={{padding:'52px 22px 16px'}}><div style={{fontSize:10,letterSpacing:3,color:'#FF85AD',fontWeight:600,marginBottom:6}}>GLOW UP</div><div style={{display:'flex',alignItems:'flex-end',justifyContent:'space-between'}}><div><div style={{fontFamily:'Playfair Display,serif',fontSize:32,fontStyle:'italic',color:'#FF2D78',lineHeight:1}}>Nutrition</div><div style={{fontSize:12,color:'#7A5068',fontWeight:300,marginTop:3}}>Eat to build</div></div><div style={{background:'rgba(255,45,120,0.1)',border:'1px solid rgba(255,45,120,0.22)',borderRadius:14,padding:'8px 14px',textAlign:'right'}}><div style={{fontSize:20,fontWeight:700,color:'#FF2D78',lineHeight:1}}>{nutTotals.kcal}</div><div style={{fontSize:8,color:'#5A3545',letterSpacing:1}}>KCAL LOGGED</div></div></div></div>
          <div style={{padding:'0 20px 14px'}}><div style={{background:'rgba(255,255,255,0.03)',border:'1px solid rgba(255,255,255,0.07)',borderRadius:16,padding:'14px 16px'}}><div style={{fontSize:8,letterSpacing:3,color:'#FF85AD',fontWeight:700,marginBottom:12}}>DAILY TARGETS</div><div style={{display:'flex',flexDirection:'column',gap:10}}>{[{label:'Protein',val:nutTotals.protein,target:NUT_TARGETS.protein,unit:'g',color:'#FF2D78'},{label:'Carbs',val:nutTotals.carbs,target:NUT_TARGETS.carbs,unit:'g',color:'#FF85AD'},{label:'Fat',val:nutTotals.fat,target:NUT_TARGETS.fat,unit:'g',color:'#FFB347'},{label:'Fibre',val:nutTotals.fibre,target:NUT_TARGETS.fibre,unit:'g',color:'#A8D8A8'}].map(function(m){var p=Math.min(m.val/m.target,1);return <div key={m.label}><div style={{display:'flex',justifyContent:'space-between',marginBottom:4}}><div style={{fontSize:11,color:'#9A7080'}}>{m.label}</div><div style={{fontSize:11,fontWeight:600,color:p>=1?m.color:'#F5E8EF'}}>{m.val}<span style={{fontSize:9,color:'#4A2535'}}>/{m.target}{m.unit}</span></div></div><div style={{height:4,borderRadius:2,background:'rgba(255,255,255,0.06)'}}><div style={{height:'100%',borderRadius:2,background:'linear-gradient(90deg,'+m.color+','+m.color+'88)',width:(p*100)+'%',transition:'width 0.45s ease'}}/></div></div>; })}</div></div></div>
          <div style={{padding:'0 20px 14px'}}><div style={{background:'rgba(100,180,255,0.05)',border:'1px solid rgba(100,180,255,0.16)',borderRadius:16,padding:'14px 16px'}}><div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:12}}><div><div style={{fontSize:8,letterSpacing:3,color:'#64B4FF',fontWeight:700,marginBottom:2}}>WATER</div><div style={{fontSize:12,color:'#7A9AAA',fontWeight:300}}>{waterMl}ml / 2,500ml</div></div>{waterMl>=2500&&<div style={{background:'rgba(100,180,255,0.18)',borderRadius:10,padding:'4px 10px',fontSize:9,fontWeight:700,color:'#64B4FF'}}>2.5L done ✓</div>}</div><div style={{display:'flex',gap:4}}>{WATER_STEPS.map(function(step,i){var filled=waterMl>=step;return <div key={step} className='tappable' onClick={function(){setWaterMl(filled&&waterMl===step?WATER_STEPS[i-1]||0:step);}} style={{flex:1,height:36,borderRadius:8,background:filled?'linear-gradient(180deg,#64B4FF,#3090E0)':'rgba(100,180,255,0.08)',border:filled?'1px solid #64B4FF':'1px solid rgba(100,180,255,0.18)',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',transition:'all 0.2s ease'}}><div style={{fontSize:filled?14:10,lineHeight:1}}>{filled?'💧':''}</div><div style={{fontSize:7,color:filled?'white':'rgba(100,180,255,0.4)',fontWeight:600,marginTop:2}}>{step/1000}L</div></div>; })}</div><div style={{marginTop:8,height:3,borderRadius:2,background:'rgba(255,255,255,0.05)'}}><div style={{height:'100%',borderRadius:2,background:'linear-gradient(90deg,#64B4FF,#90D0FF)',width:((waterMl/2500)*100)+'%',transition:'width 0.4s ease'}}/></div></div></div>
          <div style={{padding:'0 20px 14px'}}><div style={{display:'flex',background:'rgba(255,255,255,0.04)',borderRadius:12,padding:4,border:'1px solid rgba(255,255,255,0.06)'}}>{[{id:'weekday',label:'Mon-Fri'},{id:'weekend',label:'Weekend'},{id:'rules',label:'Rules'}].map(function(t){return <div key={t.id} className='tappable' onClick={function(){setNutView(t.id);}} style={{flex:1,padding:'8px 4px',borderRadius:9,textAlign:'center',background:nutView===t.id?'linear-gradient(135deg,#FF2D78,#CC0044)':'transparent',fontSize:11,fontWeight:nutView===t.id?700:400,color:nutView===t.id?'white':'#6A4055'}}>{t.label}</div>; })}</div></div>
          {nutView!=='rules'&&renderNutritionMeals()}
          {nutView==='rules'&&<div style={{padding:'0 20px'}}><div style={{marginBottom:12}}><div style={{fontSize:8,letterSpacing:3,color:'#A8D8A8',fontWeight:700,marginBottom:8,paddingLeft:2}}>GUT HEALTH — DAILY</div><div style={{background:'rgba(168,216,168,0.05)',border:'1px solid rgba(168,216,168,0.16)',borderRadius:16}}>{GUT_HEALTH.map(function(g,i){return <div key={i} style={{display:'flex',gap:12,padding:'12px 14px',alignItems:'center',borderBottom:i<GUT_HEALTH.length-1?'1px solid rgba(255,255,255,0.04)':'none'}}><div style={{fontSize:18}}>{g.emoji}</div><div style={{fontSize:13,color:'#9AA890',fontWeight:300,lineHeight:1.4}}>{g.item}</div></div>; })}</div></div><div style={{marginBottom:12}}><div style={{fontSize:8,letterSpacing:3,color:'#FF85AD',fontWeight:700,marginBottom:8,paddingLeft:2}}>GOLDEN RULES</div><div style={{display:'flex',flexDirection:'column',gap:7}}>{GOLDEN_RULES.map(function(rule,i){return <div key={i} style={{background:'rgba(255,255,255,0.04)',border:'1px solid rgba(255,255,255,0.07)',borderRadius:12,padding:'11px 14px',display:'flex',gap:11,alignItems:'flex-start'}}><div style={{width:20,height:20,borderRadius:'50%',flexShrink:0,background:'rgba(255,45,120,0.14)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:9,fontWeight:700,color:'#FF2D78'}}>{i+1}</div><div style={{fontSize:13,color:'#C49AAA',fontWeight:300,lineHeight:1.4}}>{rule}</div></div>; })}</div></div><div style={{background:'rgba(255,45,120,0.07)',border:'1px solid rgba(255,45,120,0.18)',borderRadius:14,padding:'16px',marginBottom:12}}><div style={{fontSize:11,fontWeight:700,color:'#FF2D78',marginBottom:8}}>On building muscle</div><div style={{fontSize:12,color:'#9A7080',fontWeight:300,lineHeight:1.6}}>Build muscle first. Eat train build. The results come after you have built something worth showing.</div></div></div>}
        </div>}

        {tab==='rules' && <div style={{padding:'0 15px',animation:'up .3s ease-out'}}>
          <div style={{textAlign:'center',padding:'32px 0 20px'}}><div style={{fontSize:'2.5rem',marginBottom:8}}>✅</div><h2 style={{fontFamily:'Playfair Display,serif',fontWeight:900,fontSize:'2rem',background:'linear-gradient(135deg,'+T.a1+','+T.a2+')',WebkitBackgroundClip:'text',WebkitTextFillColor:'transparent',lineHeight:1.1}}>Daily Rules</h2><p style={{color:'rgba(255,179,217,.4)',fontStyle:'italic',margin:'6px 0 0',fontSize:'0.88rem',fontFamily:'Crimson Text,serif'}}>non-negotiable every single day ✨</p></div>
          <div style={{display:'flex',gap:10,marginBottom:16}}><div style={Object.assign({},cardS,{flex:1,padding:'13px',textAlign:'center'})}><div style={{fontSize:'1.4rem',fontFamily:'Playfair Display,serif',fontWeight:900,color:T.a1}}>{rulesTodayDone} / {allRules.length}</div><div style={{color:'rgba(255,179,217,.4)',fontSize:'0.62rem',textTransform:'uppercase',letterSpacing:'0.1em',marginTop:3}}>Done Today</div></div><div style={Object.assign({},cardS,{flex:1,padding:'13px',textAlign:'center'})}><div style={{fontSize:'1.4rem',fontFamily:'Playfair Display,serif',fontWeight:900,color:'#ffb347'}}>{st.rulesStreak||0} 🔥</div><div style={{color:'rgba(255,179,217,.4)',fontSize:'0.62rem',textTransform:'uppercase',letterSpacing:'0.1em',marginTop:3}}>Day Streak</div></div></div>
          <div style={Object.assign({},cardS,{padding:'13px 16px',marginBottom:16})}><div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:7}}><span style={{color:'rgba(255,179,217,.44)',fontSize:'0.64rem',textTransform:'uppercase',letterSpacing:'0.12em'}}>Progress</span><span style={{color:T.a1,fontWeight:'bold',fontFamily:'Playfair Display,serif'}}>{rulesTodayDone} / {allRules.length}</span></div><div style={{height:7,background:'rgba(255,255,255,.05)',borderRadius:7,overflow:'hidden'}}><div style={{height:'100%',width:(allRules.length?(rulesTodayDone/allRules.length)*100:0)+'%',background:'linear-gradient(90deg,'+T.a1+','+T.a2+')',borderRadius:7,transition:'width .6s'}}/></div></div>
          <div style={{display:'flex',flexDirection:'column',gap:10}}>{allRules.map(function(rule,i){var key=today+'-'+rule.id;var done=!!st.rulesChecked[key];return <div key={rule.id} style={Object.assign({},cardS,{padding:'16px',border:done?'1px solid '+T.a1+'36':'1px solid rgba(255,120,190,.13)',background:done?T.a1+'07':'rgba(255,255,255,.035)',animation:'popIn .3s '+(i*.05)+'s ease-out both'})}><div style={{display:'flex',alignItems:'flex-start',gap:12}}><button onClick={function(){var nk=Object.assign({},st.rulesChecked,{[key]:!done});var dc=allRules.filter(function(r){return !!nk[today+'-'+r.id];}).length;var adn=dc===allRules.length;setSt(function(p){var ns2=p.rulesStreak||0;var nld=p.rulesLastDate;if(adn&&p.rulesLastDate!==today){var yest=new Date();yest.setDate(yest.getDate()-1);ns2=p.rulesLastDate===yest.toDateString()?(p.rulesStreak||0)+1:1;nld=today;}var cd=p.ch.filter(function(c){return c.done;}).length;var ip=adn&&cd>=3;var ns=Object.assign({},p,{rulesChecked:nk,rulesStreak:ns2,rulesLastDate:nld,perfectDays:ip&&p.rulesLastDate!==today?(p.perfectDays||0)+1:p.perfectDays});return Object.assign({},ns,checkAch(ns));});}} style={{width:36,height:36,minWidth:36,borderRadius:'50%',border:done?'none':'2px solid rgba(255,120,190,.34)',background:done?grad:'transparent',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',fontSize:'0.9rem',color:done?'#fff':'transparent',touchAction:'manipulation',flexShrink:0}}>{done?'✓':''}</button><div style={{flex:1,minWidth:0}}><div style={{display:'flex',alignItems:'center',gap:8,marginBottom:4}}><span style={{fontSize:'1.1rem'}}>{rule.emoji}</span><span style={{fontFamily:'Playfair Display,serif',fontWeight:'bold',fontSize:'0.95rem',color:done?'rgba(255,179,217,.32)':'#ffe8f5',textDecoration:done?'line-through':'none'}}>{rule.title}</span></div><p style={{margin:0,color:done?'rgba(255,179,217,.22)':'rgba(255,179,217,.55)',fontSize:'0.84rem',fontFamily:'Crimson Text,serif',lineHeight:1.6}}>{rule.description}</p></div></div></div>; })}</div>
          <div style={{marginTop:16,padding:'13px 16px',background:'rgba(255,255,255,.02)',borderRadius:14,border:'1px solid rgba(255,120,190,.08)',textAlign:'center'}}><div style={{color:'rgba(255,179,217,.3)',fontSize:'0.72rem',fontFamily:'Crimson Text,serif',lineHeight:1.6}}>These rules are not optional. They are who you are becoming.</div></div>
        </div>}

        {tab==='rewards' && <div style={{padding:'0 15px',animation:'up .3s ease-out'}}>
          <div style={{textAlign:'center',padding:'32px 0 20px'}}><div style={{fontSize:'2.5rem',marginBottom:8}}>🎁</div><h2 style={{fontFamily:'Playfair Display,serif',fontWeight:900,fontSize:'2rem',background:'linear-gradient(135deg,'+T.a1+','+T.a2+')',WebkitBackgroundClip:'text',WebkitTextFillColor:'transparent',lineHeight:1.1}}>Rewards</h2><p style={{color:'rgba(255,179,217,.4)',fontStyle:'italic',margin:'6px 0 0',fontSize:'0.88rem',fontFamily:'Crimson Text,serif'}}>spend your points on something special ✨</p></div>
          <div style={{display:'flex',gap:10,marginBottom:18}}><div style={Object.assign({},cardS,{flex:1,padding:'14px',textAlign:'center'})}><div style={{fontSize:'1.6rem',fontFamily:'Playfair Display,serif',fontWeight:900,color:T.a1}}>{st.glowPoints}</div><div style={{color:'rgba(255,179,217,.4)',fontSize:'0.62rem',textTransform:'uppercase',letterSpacing:'0.1em',marginTop:3}}>✨ Glow Points</div></div><div style={Object.assign({},cardS,{flex:1,padding:'14px',textAlign:'center'})}><div style={{fontSize:'1.6rem',fontFamily:'Playfair Display,serif',fontWeight:900,color:'#ff3d9a'}}>{st.grindPoints}</div><div style={{color:'rgba(255,179,217,.4)',fontSize:'0.62rem',textTransform:'uppercase',letterSpacing:'0.1em',marginTop:3}}>💪 Grind Points</div></div></div>
          {renderRewardsList()}
        </div>}
