  ...
  useEffect(function() {
    loadFromSupabase().then(function(remote) {
      if (remote && remote.total !== undefined) {
        var isNewDay = remote.lgd !== today;
        setSt(function(p) {
          var merged = Object.assign({}, p, remote);
          if (isNewDay) {
            var all = CHALLENGES.concat(remote.custom || []);
            var plan = (remote.wplan || {})[today];
            var picked;
            if (plan && plan.length > 0) {
              picked = plan.map(function(id){ return all.find(function(c){return c.id===id;}); }).filter(Boolean).slice(0,3);
            }
            if (!picked || picked.length === 0) picked = pickChallenges(remote.seen || [], remote.total || 0, remote.ovr || {}, remote.custom || []);
            merged.ch = picked.map(function(c,i){ return Object.assign({},c,{uid:today+'-'+i,done:false,note:'',diff:null}); });
            merged.lgd = today;
          }
          return merged;
        });
      ...
      {st.showPin && (
        <OVL onClose={function(){setSt(function(p){return Object.assign({},p,{showPin:false});});}}>
          <div style={Object.assign({},cardS,{padding:'36px 24px',textAlign:'center',animation:'popIn .3s ease-out',border:'1px solid '+T.a1+'44'})}>
            ...
            <input type='tel' inputMode='numeric' pattern='[0-9]*' autoComplete='off' placeholder='••••' value={st.pi} onChange={function(e){setSt(function(p){return Object.assign({},p,{pi:e.target.value.replace(/[^0-9]/g,''),pe:''});});}} onKeyDown={function(e){if(e.key==='Enter')submitPin();}} style={Object.assign({},inpS,{textAlign:'center',fontSize:'2rem',letterSpacing:'0.6em',marginBottom:12})}/>
